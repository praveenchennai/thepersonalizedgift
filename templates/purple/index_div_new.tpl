...
<body>
  ...
  <div class="headerbox"> <!--header box starts-->
       <div class="logo"> 
        <div class="menubg">
...
<!-- <div class="createpersonalizedgift"> {if $WELCOME_MESSAGE_STORE.img_extension ne ''}<img src="{$smarty.const.SITE_URL}/modules/store/images/{$WELCOME_MESSAGE_STORE.id}.jpg" width="508" height="64" />{else}<img src="{$GLOBAL.tpl_url}/images/creat-personalized-gifts.jpg" />{/if}</div>
 --><!--<div class="createpersonalizedgift"> <img src="{$GLOBAL.tpl_url}/images/creat-personalized-gifts.jpg" /></div>
-->
<div class="createpersonalizedgift"> 
	{if $WELCOME_MESSAGE_STORE.heading1 eq ''}<img src="{$GLOBAL.tpl_url}/images/creat-personalized-gifts.jpg" />{else}
		<div class="storehead1" align="right">{$WELCOME_MESSAGE_STORE.heading1}<br/>
			<div class="storehead2" align="right">{$WELCOME_MESSAGE_STORE.heading2}</div>
		</div>
	{/if}
</div>
...
<img src="{$GLOBAL.tpl_url}/images/welcome.jpg" width="365" height="22" border="0" />{if $WELCOME_MESSAGE_STORE.description != ''}<div class="welcometext">{$WELCOME_MESSAGE_STORE.description|truncate:200}
 ...
 <span class="redtext">
  <a href="welcome.php" class="redtext">{$MOD_VARIABLES.MOD_COMM.COMM_MORE} &raquo;</a></span></div>
  {/if}
...
{if $STOREFLAG eq 'Y'}<div style="float:left; width:130px; height:18px; margin-top:5px;"><a href="{makeLink mod=member pg=register}act=add_edit{/makeLink}" class="footerlink"><b>&raquo;&nbsp;{$MOD_VARIABLES.MOD_COMM.COMM_CMN_REGISTER_TOP}</b></a></div>{else}<div style="float:left; width:130px; height:18px; margin-top:5px;"></div>{/if}
   <div style="float:left; height:18px; margin-top:5px;"><a href="{makeLink mod=member pg=pswd}{/makeLink}" class="footerlink"><b>&raquo; {$MOD_VARIABLES.MOD_COMM.COMM_CMN_FORGOT_PASSWORD_TOP}</b></a></div>   
</div>
</form>
{else}
<div class="loginbox">
...
{foreach from=$FEATURED_ITEMS item=item name=foo}
	...
	<div class="photobox">{/if}{if $item.gift eq 'gift'}<a href="{makeLink mod=product pg=list}act=listproduct&id={$item.id}&cat_id={$item.category_id}{/makeLink}">
	<img src="{$GLOBAL.mod_url}product/images/thumb/{$item.id}_List_.{$item.image_extension }" border="0"/></a>{else}<a href="{makeLink mod=product pg=list}act=pg_dettails&id={$item.id}&cat_id={$item.category_id}&product_id={$item.pid}{/makeLink}"><img src="{$GLOBAL.mod_url}ajax_editor/images/thumb/{$item.id}_1.{$item.image_extension }" border="0"/></a>{/if}</div>
	</div>
    <div class="phototext"  style="height:120px;overflow:hidden">
	<div style="height:60px;overflow:hidden">
	{$item.description|truncate:60}
      <span><a href="{makeLink mod=product pg=list}act=listproduct&id={$item.id}&cat_id={$item.category_id}{/makeLink}" class="redtext">{$MOD_VARIABLES.MOD_COMM.COMM_READ_MORE} &raquo;</a></span>
	  </div>
     <div class="price" align="left" style="padding-top:8px">
	 {if $item.price ne $item.discount_price}
	 <del style="color:#FF0000"><span class="redtext2" >{$CURRENCY_SYMBOL}{$item.price|truncate:5:""}</span></del>&nbsp;&nbsp;
     <span class="redtext3">{$CURRENCY_SYMBOL}{$item.discount_price|truncate:5:""}</span><div style="padding-top:5px">&nbsp;&nbsp;&nbsp;&nbsp;<a href="{if $item.gift eq 'gift'}{makeLink mod=product pg=list}act=listproduct&id={$item.id}&cat_id={$item.category_id}{/makeLink}{else}{makeLink mod=product pg=list}act=pg_dettails&id={$item.id}&cat_id={$item.category_id}&product_id={$item.pid}{/makeLink}{/if}"><img src="{$GLOBAL.tpl_url}/images/buy.jpg" width="61" height="22" border="0" /  align="absmiddle" /></a></div></div>
	 {else}
	 <span class="redtext2" >&nbsp;&nbsp;&nbsp;&nbsp;{$CURRENCY_SYMBOL}{$item.price|truncate:5:""}</span>
...
<span class="whitenormaltext"><a  class="whitetext" href="{makeLink mod=cart pg=default}act={if $smarty.session.SHIPPING_ADDRESS.method}checkout{else}shipping{/if}{/makeLink}"><img src="{$GLOBAL.tpl_url}/images/arrow_white.gif" width="7" height="10"  align="absbottom"style="border:0" />{$MOD_VARIABLES.MOD_COMM.COMM_CHECK_OUT}</a></span> </div>
...
<ul id="LMDIV2list">
{if $smarty.request.storename=='personaltouch'}
