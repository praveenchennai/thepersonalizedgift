...
{foreach from=$FEATURED_ITEMS item=item name=foo}
	...
	<div class="photobox">{/if}{if $item.gift eq 'pgift'}<a href="{makeLink mod=product pg=list}act=pg_details&id={$item.id}&cat_id={$item.category_id}&product_id={$item.pid}&parent_cat_id={$item.category}{/makeLink}"><img src="{$GLOBAL.mod_url}ajax_editor/images/thumb/{$item.id}_1.{$item.image_extension }" border="0"/></a>{else}<a href="{makeLink mod=product pg=list}act=listproduct&id={$item.id}&cat_id={$item.category_id}{/makeLink}">
	<img src="{$GLOBAL.mod_url}product/images/thumb/{$item.id}_List_.{$item.image_extension }" border="0"/></a>{/if}</div>
	</div>
    <div class="phototext"  style="height:120px;overflow:hidden">
	<div style="height:60px;overflow:hidden">
	{$item.description|truncate:60}
      <span><a href="{if $item.gift eq 'pgift'}{makeLink mod=product pg=list}act=pg_details&id={$item.id}&cat_id={$item.category_id}&product_id={$item.pid}&parent_cat_id={$item.category}{/makeLink}{else}{makeLink mod=product pg=list}act=listproduct&id={$item.id}&cat_id={$item.category_id}{/makeLink}{/if}" class="redtext">{$MOD_VARIABLES.MOD_COMM.COMM_READ_MORE} &raquo;</a></span>
	  </div>
     ...
     <span class="redtext3">{$CURRENCY_SYMBOL}{$item.discount_price|truncate:5:""}</span><div style="padding-top:5px">&nbsp;&nbsp;&nbsp;&nbsp;<a href="{if $item.gift eq 'pgift'}{makeLink mod=product pg=list}act=pg_details&id={$item.id}&cat_id={$item.category_id}&product_id={$item.pid}&parent_cat_id={$item.category}{/makeLink}{else}{makeLink mod=product pg=list}act=pg_details&id={$item.id}&cat_id={$item.category_id}&product_id={$item.pid}&parent_cat_id={$item.category}{/makeLink}{/if}"><img src="{$GLOBAL.tpl_url}/images/buy.jpg" width="61" height="22" border="0" /  align="absmiddle" /></a></div></div>
	 {else}
...
<ul id="LMDIV2list">
{if $smarty.request.storename=='personaltouch' or $smarty.request.storename=='newhome'}
