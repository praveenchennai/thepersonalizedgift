...
<form name="frlgfail" method="post" action="{makeLink mod=member pg=login}{/makeLink}" style="margin:0px; ">
  <div class="loginbox">
    ...
    <br/>
   <div style="float:left; width:130px;">{$MOD_VARIABLES.MOD_LABELS.LBL_USER_NAME}</div>
   <div style="float:left; width:100 px; text-align:left"> {$MOD_VARIABLES.MOD_LABELS.LBL_PASSWORD}</div><br />
...
{foreach from=$FEATURED_ITEMS item=item name=foo}
	...
	{$item.description|truncate:50}
	 ...
	 <del><span class="redtext2" >{$CURRENCY_SYMBOL}{$item.price|truncate:9:""}</span></del>&nbsp;&nbsp;
