...
function submitFormavtar(avatarid)	
{
	...
	function submitIDs(){
		...
		{/literal}	
		document.cssForm.action="{makeLink mod=$MOD pg=$PG}act=assign_css_ptp&sId={$SID}&fId={$FID}&store_id={$STORE_ID}{/makeLink}";
		{literal}
		document.cssForm.submit();
	}
{/literal}	
...
<table width="90%"  border="0" cellspacing="0" cellpadding="0" class=naBrdr>
  <tr height="30">
  ...
  <tr>
    <td height="100%" colspan="2"><table width="80%"  border="0" align="center" cellpadding="0" cellspacing="0">
		    {if count($CSS_LIST) > 0}
						 <tr>{foreach from=$CSS_LIST item=css name=foo}	
					 {if  $smarty.foreach.foo.index ne "0"  && $smarty.foreach.foo.index is div by 2}
			</tr><tr>
						{/if}
					   <td width="50%" align="center" valign="top"><img src="{$smarty.const.SITE_URL}/templates/template/images/thumb/temp_{$css->id}.gif" border="0"><img src="{$smarty.const.SITE_URL}/templates/template/images/large_{$css->id}.gif" border="0" style="display:none; ">
  ...
  {/if}
  <tr>
    <td colspan="2" align="right"><a class="linkOneActive" href="#" onClick="submitIDs();"><img src="{$GLOBAL.tpl_url}/images/btn_select.gif"></a></td>
    <td>&nbsp;</td>
    <td align="left"><a class="linkOneActive" href="{makeLink mod="store" pg="index"}act=form{/makeLink}&id={$smarty.request.store_id}&sId={$smarty.request.sId}&fId={$smarty.request.fId}&mId={$MID}" onClick="javascript:history.go(-1);"><img src="{$GLOBAL.tpl_url}/images/btn_back.gif"></a></td>
  </tr>
  <tr>
    <td colspan="2">&nbsp;</td>
    <td>&nbsp;</td>
