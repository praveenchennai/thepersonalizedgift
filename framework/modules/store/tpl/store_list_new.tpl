  ...
  <tr> 
    <td><table border="0" cellspacing="0" cellpadding="4" width="100%" align="center"> 
        ...
        <tr>
		  ...
		  <td class="naGrid1"><a href="{makeLink mod=$MOD pg=$PG}act=inactive_store_list&sId={$smarty.request.sId}&fId={$smarty.request.fId}{/makeLink}" >Disabled Retail Shop</a></td>
          ...
          <td width="5%" nowrap class="naGridTitle" align="center">{makeLink mod=store pg=index orderBy="active" display="Status"}act=list&sId={$smarty.request.sId}&fId={$smarty.request.fId}{/makeLink}</td>
		     <td  width="10%"  nowrap class="naGridTitle" align="center">{makeLink mod=store pg=index orderBy="enddate" display="Subscription"}act=list&sId={$smarty.request.sId}&fId={$smarty.request.fId}{/makeLink}</td>
			     <td  width="10%"  nowrap class="naGridTitle" align="center">IPN message</td>
				  <td  width="10%"  nowrap class="naGridTitle" align="center">Disable</td>
        </tr>
		...
		{/if}</td> 
		     <td  align="left" valign="middle">{$row->subs_status}</td> 
			  <td  align="left" valign="middle"><a class="linkOneActive" href="javascript:void(0);" onclick="popupWindow('{makeLink mod=$MOD pg=view_ipn_msg}sid={$row->id}&txn_id={$row->txn_id}{/makeLink}')"  >view</a> </td> 
			   <td  align="left" valign="middle"><a class="linkOneActive" onclick="javascript: return confirm('Are you sure to disable the store&nbsp;&nbsp;{$row->name}&nbsp;?')"  href="{makeLink mod=store pg=index}act=inactive_store_list&id={$row->id}&mem_type=2&sId={$smarty.request.sId}&fId={$smarty.request.fId}&status=Y{/makeLink}">Disable</a></td>
        </tr> 
        {/foreach}
        <tr> 
