...
$param			=	"mod=$mod&pg=$pg&act=list&orderBy=$orderBy&parent_id=$parent_id&status_id=$status_id&category_search=$category_search&keysearch=$keysearch&sId=$sId&fId=$fId";
...
switch($_REQUEST['act']) {
	case "list":
		...
		if($parent_id=="")
		$parent_id	=	0;
		list($rs, $numpad, $cnt, $limitList)	= 	$objFlyer->listMyFlyers('A',$keysearch,$flyer_search,$pageNo,$limit,$param,OBJECT, $orderBy,$status_id);
		$cur_date	=	date('Y-m-d');
