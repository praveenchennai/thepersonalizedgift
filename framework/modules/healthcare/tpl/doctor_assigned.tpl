    ...
    <td><table border=0 width=100% cellpadding="5" cellspacing="2"> 
        ...
        <tr>
		  ...
		  <td colspan="1" class="naGridTitle" height="24" align="left" width="134">{makeLink mod=`$smarty.request.mod` pg=`$smarty.request.pg` orderBy="email" display="Email"}act={$smarty.request.act}&mem_type={$smarty.request.mem_type}&sId={$smarty.request.sId}&fId={$smarty.request.fId}{/makeLink}</td> 
		   ...
		   <td colspan="1" class="naGridTitle" height="24" align="left" width="134">{makeLink mod=`$smarty.request.mod` pg=`$smarty.request.pg` orderBy="T2.username" display="Doctor"}act={$smarty.request.act}&mem_type={$smarty.request.mem_type}&sId={$smarty.request.sId}&fId={$smarty.request.fId}{/makeLink}</td> 
        ...
        <tr class="{cycle values="naGrid1,naGrid2"}" >
          <td valign="middle" height="24" align="left" {if $user->status eq 'Doctor_Assigned_Request'}bgcolor="#FF8080" {/if}><a class="linkOneActive" href="{makeLink mod=`$smarty.request.mod` pg=`$smarty.request.pg`}act=view&id={$user->medical_questionnaire_id}&assign_id={$user->assignID}&refill={$user->refill}&doct_id={$user->doctor_id}&pharma_id={$user->pharmacy_id}&stat={$user->status}&sId={$smarty.request.sId}&fId={$smarty.request.fId}&pageNo={$smarty.request.pageNo}{/makeLink}">{$user->username}</a></td> 
		  <td valign="middle" height="24" align="left" {if $user->status eq 'Doctor_Assigned_Request'} bgcolor="#FF8080" {/if}>{$user->first_name}</td> 
		   <td valign="middle"  height="24" align="left" {if $user->status eq 'Doctor_Assigned_Request'} bgcolor="#FF8080" {/if}>{$user->last_name}</td> 
		    <td height="24" align="left" valign="middle" {if $user->status eq 'Doctor_Assigned_Request'} bgcolor="#FF8080" {/if}>{$user->email}</td> 
		<!--	  <td  height="24" align="left" valign="middle">{$DOCTOR_LIST[$smarty.foreach.foo.index]->doctor}</td> -->
		    <td  height="24" align="left" valign="middle" {if $user->status eq 'Doctor_Assigned_Request'} bgcolor="#FF8080" {/if}>{$user->DoctorName}</td>
			<td  height="24" align="left" nowrap valign="middle" {if $user->status eq 'Doctor_Assigned_Request'} bgcolor="#FF8080" {/if}>{$user->assigned_date|date_format:"%m-%d-%Y %H:%M:%S"}</td>
			 <td  height="24" align="left" valign="middle" {if $user->status eq 'Doctor_Assigned_Request'} bgcolor="#FF8080" {/if}>{if $user->refill mod 3 ==0 AND $user->refill>0}Reconsult{/if}
