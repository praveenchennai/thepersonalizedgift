...
function deleteSelected()
{
	...
	{
		document.frm_category.action='{/literal}{makeLink mod=`$smarty.request.mod` pg=`$smarty.request.pg`}act=delete&limit={$smarty.request.limit}&category_search={$smarty.request.category_search}&id={$smarty.request.id}{/makeLink}{literal}'; 
		document.frm_category.submit();
	}
}

function deleteSelectedTwo(catid){
	document.frm_category.action='{/literal}{makeLink mod=`$smarty.request.mod` pg=`$smarty.request.pg`}act=delete&limit={$smarty.request.limit}&CATID='+catid+'&category_search={$smarty.request.category_search}&id={$smarty.request.id}{/makeLink}{literal}'; 
	document.frm_category.submit();
}
...
function moduleChange() 
{
	...
	document.frm_category.action='{/literal}{makeLink mod=`$smarty.request.mod` pg=`$smarty.request.pg`}act=list&limit={$smarty.request.limit}{/makeLink}&id='+ id +'&category_search={$smarty.request.category_search}{literal}'; 
	document.frm_category.submit();
}
	...
	{if $DELETE_ALL eq 1}
		<tr>
			<td align="center">
				<table>
					<tr>
						<td align="center"><input name="btn_delete_all" type="button" class="naBtn" value="Yes" onClick="javascript: deleteSelectedTwo({$CATID});"></td>
						<td align="center"><input name="btn_delete_all" type="button" class="naBtn" value="No" onClick="javascript: deleteCancel();"></td>
					</tr>
				</table>
			</td>
		</tr>
...
<table align="center" width="80%" border="0" cellspacing="0" cellpadding="0" class=naBrdr> 
  <tr> 
	...
	{if $STORE_PERMISSION.add == 'Y'}
		  {if $GLOBAL.category_add_del ne "N"}
		  <a href="{makeLink mod=`$smarty.request.mod` pg=`$smarty.request.pg`}act={$ACT}&store_id={$smarty.request.store_id}&parent_id={$smarty.request.parent_id}{/makeLink}&sId={$smarty.request.sId}&fId={$smarty.request.fId}&mId={$MID}&limit={$smarty.request.limit}&category_search={$smarty.request.category_search}&id={$smarty.request.id}">Add New {$smarty.request.sId}</a>
		  {/if}
	 {/if}
		  </td> 
        </tr> 
    ...
    {/if}
		...
		<td> 
		  ...
		  {foreach from=$MODULECATEGORY item=modcategory name=foo}
      	  <option value="{$modcategory.id}" {if $smarty.request.id eq $modcategory.id}selected{/if}>{$modcategory.name}</option>
		  {/foreach}
		 </select>
	    ...
	    <tr>
          ...
          <td width="30%" align="left" nowrap class="naGridTitle">{makeLink mod=`$smarty.request.mod` pg=`$smarty.request.pg` orderBy=category_name display="`$smarty.request.sId` Name"}act=list&parent_id={$smarty.request.parent_id}&sId={$smarty.request.sId}&fId={$smarty.request.fId}&store_id={$smarty.request.store_id}&id={$smarty.request.id}&category_search={$smarty.request.category_search}{/makeLink}</td> 
          {if $FIELDS.5==Y}
		   ...
		   {/if}
		   {if $FIELDS.4==Y}
         <td width="30%" align="left" nowrap class="naGridTitle">{makeLink mod=`$smarty.request.mod` pg=`$smarty.request.pg` orderBy=display_order display="Display Order"}act=list&parent_id={$smarty.request.parent_id}&sId={$smarty.request.sId}&fId={$smarty.request.fId}&store_id={$smarty.request.store_id}{/makeLink}</td>
        ...
        <tr class="{cycle values="naGrid1,naGrid2"}">
          ...
          <td align="left" valign="middle"><a class="linkOneActive" href="{makeLink mod=`$smarty.request.mod` pg=`$smarty.request.pg`}act={$ACT}&category_id={$category->category_id}&parent_id={$category->parent_id}&sId={$smarty.request.sId}&fId={$smarty.request.fId}&limit={$smarty.request.limit}&store_id={$smarty.request.store_id}&bulk={$category->is_in_ui}&category_search={$smarty.request.category_search}{/makeLink}">{$category->category_name} </a></td> 
      	 {if $FIELDS.5==Y}
	      <td align="left" valign="middle">{if ($category->is_in_ui eq Y)} YES  {else} NO {/if}</td>
		 {/if}
		   {if $FIELDS.4==Y}
		            <td align="left" valign="middle">{$category->display_order}</td>
			{/if}
...
<td align="right"><strong>Append  to the existing </strong><input name="append" type="checkbox" id="append" value="Y" checked></td>
  ...
  <tr>
    ...
    </tr><tr><td colspan=2 valign=center>&nbsp;</td></tr></table>
