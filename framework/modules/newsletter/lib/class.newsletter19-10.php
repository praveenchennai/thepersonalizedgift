...
class Newsletter extends FrameWork {
    ...
    function newsletterScheduleAdd(&$req) {
        ...
        if(!trim($sender_name) || !trim($sender_email)) {
            ...
            $criteria = "email={$req['email']}&format={$req['format']}&confirmed={$req['confirmed']}&active={$req['active']}";
    ...
    function send($id, $page) {
        ...
        if(($count = $this->newsletterCount(array("list_id"=>$list_id, "email"=>$email, "format"=>$format, "confirmed"=>$confirmed, "active"=>$active))) > $page) {
            $sendArr['status'] = 'P';
