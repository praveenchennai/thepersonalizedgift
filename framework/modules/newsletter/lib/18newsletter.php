        ...
        if($_SERVER['REQUEST_METHOD'] == "POST") {
            if ($req['step'] == 1) {
                if( is_numeric($message = $newsletter->newsletterCount($req)) ) {
                    if( $message > 0 ) {
                        redirect(makeLink(array("mod"=>"newsletter", "pg"=>"newsletter"), "act=send&id={$req['id']}&list_id={$req['list_id']}&email={$req['email']}&format={$req['format']}&confirmed={$req['confirmed']}&active={$req['active']}&country={$req['country']}&all_users={$req['all_users']}&step=2&count=$message"));
                    } else {
