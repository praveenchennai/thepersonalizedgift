...
class Newsletter extends FrameWork {
    ...
    function newsletterAddEdit (&$req) {
	 ...
	 function newsletterEmails($country='',$active='',$confirmed='',$reg_pack='',$sub_pack='',$date_from='',$date_to,$email='',$active_store='')
	 {
	 	
		//$schedule = $this->newsletterScheduleGet($id);
		//parse_str($schedule['criteria']);
		$condition = "";
				if($country!="") $condition = " AND m.country ='$country'";
				if($active !="") $condition = $condition." AND m.active LIKE '%$active%'";
				//if($confirmed!="") $condition = $condition." AND s.deleted = '$confirmed'";
				...
				if($active_store!="") $condition = $condition."AND s.deleted = '$active_store'";
				
		$sql="SELECT  m.email  FROM `member_master` m LEFT JOIN `member_address` ma on m.id=ma.user_id and ma.addr_type='master' left join country_master c ON(ma.country = c.country_id) LEFT JOIN store s on s.user_id=m.id left join member_subscription ms ON ms.user_id=m.id  where (m.mem_type='2' ) AND m.from_store='0' AND m.email LIKE '%$email%' ".$condition."";
...
//Modified:07/11/2007  By Ratheesh KK
    function newsletterCount($req) {
		...
		$condition = "";
				...
				if($req[active]!="") $condition = $condition."AND m.active LIKE '%$req[active]%'";
				//if($req[confirmed]!="") $condition = $condition."AND s.deleted = '$req[confirmed]'";
				...
				if($req[active_store]!="") $condition = $condition."AND s.deleted = '$req[active_store]'";
				
		$sql="SELECT  m.*,s.deleted  FROM `member_master` m LEFT JOIN `member_address` ma on m.id=ma.user_id and ma.addr_type='master' left join country_master c ON(ma.country = c.country_id) LEFT JOIN store s on s.user_id=m.id left join member_subscription ms ON ms.user_id=m.id  where (m.mem_type='2' ) AND m.from_store='0' AND m.email LIKE '%$email%' ".$condition.""; 
    ...
    function newsletterScheduleAdd(&$req) {
        ...
        if(!trim($sender_name) || !trim($sender_email)) {
            ...
            $criteria = "email={$req['email']}&format={$req['format']}&confirmed={$req['confirmed']}&active={$req['active']}&country={$req['country']}&all_users={$req['all_users']}&reg_pack={$req['reg_pack']}&sub_pack={$req['sub_pack']}&date_from={$req['date_from']}&date_to={$req['date_to']}&active_store={$req['active_store']}";
    ...
    function send($id, $page) {
        ...
        $newsSubs = $this->db->get_row("SELECT * FROM newsletter_subscription WHERE list_id = '$list_id' AND email LIKE '%$email%' AND format LIKE '%$format%' AND confirmed LIKE '%$confirmed%' AND active LIKE '%$active%' LIMIT $page, 1", ARRAY_A);
			...
			$newsSubs = $this->db->get_row("SELECT  m.*,s.deleted  FROM `member_master` m LEFT JOIN `member_address` ma on m.id=ma.user_id and ma.addr_type='master' left join country_master c ON(ma.country = c.country_id) LEFT JOIN store s on s.user_id=m.id left join member_subscription ms ON ms.user_id=m.id  where (m.mem_type='2' ) AND m.from_store='0' AND m.email LIKE '%$email%' ".$condition."  LIMIT $page, $fIntervel" , ARRAY_A);
			...
			$countnew = $this->db->get_var("SELECT  COUNT(*)  FROM `member_master` m LEFT JOIN `member_address` ma on m.id=ma.user_id and ma.addr_type='master' left join country_master c ON(ma.country = c.country_id) LEFT JOIN store s on s.user_id=m.id left join member_subscription ms ON ms.user_id=m.id  where (m.mem_type= '2' ) AND m.from_store='0' AND m.email LIKE '%$email%' ".$condition." "); 
		...
		if($newsSubs) {
		 if ($list_id>0){
			...
			if ($this->db->get_var("SELECT member_id FROM newsletter_log WHERE member_id = '{$newsSubs['user_id']}' AND newsletter_id = '$newsletter_id'")) {
                ...
                $array = array("subscription_id"=>$newsSubs['id'],"member_id"=>$newsSubs['id'], "newsletter_id"=>$newsletter_id, "schedule_id"=>$id, "date_sent"=>date("Y-m-d H:i:s"));
        ...
        if(($count = $this->newsletterCount(array("list_id"=>$list_id, "reg_pack"=>$reg_pack, "sub_pack"=>$sub_pack, "email"=>$email, "format"=>$format, "confirmed"=>$confirmed, "active"=>$active, "country"=>$country, "all_users"=>$all_users, "date_from"=>$date_from, "date_to"=>$date_to, "active_store"=>$active_store))) > $page) {
            $sendArr['status'] = 'P';
   ...
   function supportList ($pageNo, $limit = 10, $params='', $output=OBJECT, $orderBy) {
	...
	function sendAjax($id, $page, $batch_email, $delayed_seconds) {
        ...
        $newsSubsArr = $this->db->get_results("SELECT * FROM newsletter_subscription WHERE list_id = '$list_id' AND email LIKE '%$email%' AND format LIKE '%$format%' AND confirmed LIKE '%$confirmed%' AND active LIKE '%$active%' LIMIT $page, $fIntervel", ARRAY_A);
			...
			$newsSubsArr = $this->db->get_results("SELECT  m.*,s.deleted  FROM `member_master` m LEFT JOIN `member_address` ma on m.id=ma.user_id and ma.addr_type='master' left join country_master c ON(ma.country = c.country_id) LEFT JOIN store s on s.user_id=m.id left join member_subscription ms ON ms.user_id=m.id  where (m.mem_type='2' ) AND m.from_store='0' AND m.email LIKE '%$email%' ".$condition."  LIMIT $page, $fIntervel" , ARRAY_A);
...
function userSubscriptionAddEdit ($req) {
	...
	# Function to check whther the Mailing List menu is enabled or not
	# Added By Arun
	function checkMailingListisEnabled()
	{
		$sql = "select active from module_menu where menu = 'Mailing List'";
		$rs = $this->db->get_row($sql, ARRAY_A);
		return $rs;
	}
	
	# Function to check store module enabled
	# added by arun
	function checkStoreEnabled()
	{
		$sql = "select active from module where folder = 'store'";
		$rs = $this->db->get_row($sql, ARRAY_A);
		return $rs;
	}
