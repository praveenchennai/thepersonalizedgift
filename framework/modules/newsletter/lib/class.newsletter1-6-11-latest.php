...
class Newsletter extends FrameWork {
    ...
    function newsletterAddEdit (&$req) {
	 ...
	 function newsletterEmails($country='',$active='',$confirmed='',$reg_pack='',$sub_pack='',$date_from='',$date_to,$email='')
	 {
...
//Modified:07/11/2007  By Ratheesh KK
    ...
    function send($id, $page) {
        ...
        $newsSubs = $this->db->get_row("SELECT * FROM newsletter_subscription WHERE list_id = '$list_id' AND email LIKE '%$email%' AND format LIKE '%$format%' AND confirmed LIKE '%$confirmed%' AND active LIKE '%$active%' LIMIT $page, 1", ARRAY_A);
			...
			$countnew = $this->db->get_var("SELECT  m.*,s.deleted  FROM `member_master` m LEFT JOIN `member_address` ma on m.id=ma.user_id and ma.addr_type='master' left join country_master c ON(ma.country = c.country_id) LEFT JOIN store s on s.user_id=m.id left join member_subscription ms ON ms.user_id=m.id  where (m.mem_type= '2' ) AND m.from_store='0' AND m.email LIKE '%$email%' ".$condition." ");
