	...
	*/
	...
	//print_r($req);exit;
		...
		*/
		...
		elseif (!preg_match($pattern, $resize_sn_senti_no)) {
			 $message 				=	"Please Enter Valid Characters Only in Resize % Fields";
			 
		}
		elseif (!preg_match($pattern, $resize_sn_senti_yes)) {
			 $message 				=	"Please Enter Valid Characters Only in Resize % Fields";
			 
		}
		elseif (!preg_match($pattern, $resize_dn_senti_no)) {
			 $message 				=	"Please Enter Valid Characters Only in Resize % Fields";
			 
		}
		elseif (!preg_match($pattern, $resize_dn_senti_yes)) {
			 $message 				=	"Please Enter Valid Characters Only in Resize % Fields";
			...
			*/
			...
			if($id)
			{
				...
				if ($store_id !=''){//If editing from the store/manage...
					...
					if($old_accessory_det)//If the accessory was already edited...update it changeed $old_accessory_det =='1' to  $old_accessory_det by adarsh
						{
							$arr[0]["id"] = $id;
					...
					//changeg by adarsh for storeing the old product id
					$arr[0]["parent_id"] = $id;
	...
	function listAllAccessoriesOfCatagorybySearch2($pageNo=0, $limit = 10, $params='', $output=OBJECT, $orderBy,	$category_id='',$accessory_search,$string='',$store_id='')
		{
			...
			if ($store_id !='')
			{
				$qry	=	"SELECT DISTINCT pa.id,pa.name,pa.display_name,pa.category_id,pa.adjust_weight ,pa.type,pa.cart_name,$qry1,b.store_id,b.accessory_id,b.active
		            FROM  product_accessories pa $join_qry
		 			INNER JOIN category_accessory ca ON pa.id = ca.accessory_id AND ca.category_id in ($string)
		            ,store_accessory b  
					WHERE  pa.id = b.accessory_id
