...
<table align="center" width="80%" border="0" cellspacing="0" cellpadding="0" class=naBrdr> 
  <tr> 
    <td><table width="98%" align="center"> 
        <tr> 
		  ...
		  {if $GLOBAL.productgroup_add_del ne "N"}
			  <a href="{makeLink mod=$MOD pg=$PG}act=settingsForm&limit={$smarty.request.limit}&sId={$smarty.request.sId}&fId={$smarty.request.fId}{/makeLink}">
			  Add New {$smarty.request.sId}
			  </a>
		  {/if}
		  {/if}</td> 
        </tr> 
  ...
  <tr>
	...
	{if $GLOBAL.productgroup_add_del ne "N"}
		<a class="linkOneActive" href="#" onclick="javascript: document.frm_product_group.action='{makeLink mod=$MOD pg=$PG}act=settings_Delete&fId={$smarty.request.fId}&sId={$smarty.request.sId}&limit={$smarty.request.limit}{/makeLink}'; document.frm_product_group.submit();">
		Delete
		</a>
	{/if}	
	{/if}
    ...
    {/if}
       {if count($SETTINGS_LIST) > 0}
        ...
        <tr class="{cycle values="naGrid1,naGrid2"}">
          ...
          <td align="left" valign="middle"><a class="linkOneActive" href="{makeLink mod=$MOD pg=$PG}act=settingsForm&id={$product->id}&fId={$smarty.request.fId}&sId={$smarty.request.sId}&limit={$smarty.request.limit}{/makeLink}">{$product->name} </a></td> 
          <td align="left" valign="middle"><a href="{makeLink mod=$MOD pg=$PG}act=settings_Form&id={$product->id}&fId={$smarty.request.fId}&sId={$smarty.request.sId}&limit={$smarty.request.limit}{/makeLink}">View Products</a></td>
        </tr> 
        {/foreach}
        <tr> 
