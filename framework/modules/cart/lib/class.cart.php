...
class Cart extends FrameWork {	
	...
	function deleteProductCustomFieldValues($cart_id)
	{
	    $sql	= 	" DELETE FROM product_custom_field_values where cart_id = '$cart_id'";
		$rs 	= 	$this->db->query($sql);
		return true;;
	}		
	
	function insertCustomFieldValues($cart_id,$product_id)
	{

		//----------------- custom fields ------------------//
		$text_array 		= 	get_magic_quotes_gpc() ?   stripslashes($_REQUEST['text_array']) : $_REQUEST['text_array'];
		$text_area_array 	= 	get_magic_quotes_gpc() ?   stripslashes($_REQUEST['text_area_array']) : $_REQUEST['text_area_array'];
		$select_array 		= 	get_magic_quotes_gpc() ?   stripslashes($_REQUEST['select_array']) : $_REQUEST['select_array'];
		$radio_array 		= 	get_magic_quotes_gpc() ?   stripslashes($_REQUEST['radio_array']) : $_REQUEST['radio_array'];
		
		$text_array 		= json_decode($text_array,true);
		$text_area_array 	= json_decode($text_area_array,true);
		$select_array 		= json_decode($select_array,true);
		$radio_array 		= json_decode($radio_array,true);
		
		//print_r($text_array);
		//print_r($text_area_array);
		...
		$flg	=	$this->deleteProductCustomFieldValues($cart_id);
		
		foreach($text_array  as $item){
			$array			=	array("cart_id" => $cart_id, "product_id" => $product_id, "field_id" => $item['field_id'], "field_type" => "text", "field_name" => mysql_real_escape_string(str_replace(array(': *',':',' *'),'',$item['field_name'])), "field_value" => mysql_real_escape_string($item['field_value']), "created_date" => date('Y-m-d H:i:s'),"note" => '');
			if( $item['field_value'] )
				$this->db->insert("product_custom_field_values", $array );
		}
		
		foreach($text_area_array  as  $item){
			$array			=	array("cart_id" => $cart_id, "product_id" => $product_id, "field_id" => $item['field_id'], "field_type" => "text_area", "field_name" => mysql_real_escape_string(str_replace(array(': *',':',' *'),'',$item['field_name'])), "field_value" => mysql_real_escape_string($item['field_value']), "created_date" => date('Y-m-d H:i:s'),"note" => '');
			if( $item['field_value'] )
				$this->db->insert("product_custom_field_values", $array );
		}
		
		foreach($select_array  as  $item){
			$array			=	array("cart_id" => $cart_id, "product_id" => $product_id, "field_id" => $item['field_id'], "field_type" => "select", "field_name" => mysql_real_escape_string(str_replace(array(': *',':',' *'),'',$item['field_name'])), "field_value" => mysql_real_escape_string($item['field_value']), "created_date" => date('Y-m-d H:i:s'),"note" => '');
			if( $item['field_value'] )
				$this->db->insert("product_custom_field_values", $array );
		}
		
		foreach($radio_array  as  $item){
			$array			=	array("cart_id" => $cart_id, "product_id" => $product_id, "field_id" => $item['field_id'], "field_type" => "radio", "field_name" => mysql_real_escape_string(str_replace(array(': *',':',' *'),'',$item['field_name'])), "field_value" => mysql_real_escape_string($item['field_value']), "created_date" => date('Y-m-d H:i:s'),"note" => '');
    ...
    function addToCart(&$req) {
    	...
    	extract($req);  
			...
			if(!$price) {
				...
				/*if($global['minimum_price']){
					if($total_price *$qty < $global['minimum_price'])
						$total_price =$global['minimum_price'];
			...
			//print_r($this->config['minimum_price']);exit;
			### This Code is added for setting Minimum Price...
			### Author Jipson Thomas...........................
			### Dated  31/ March 2008..........................
			if($this->config['minimum_price']){
				//print_r($product_price);exit;
				$minprodprice = $product_price*$qty*$width*$height;
			}
			
			### End of Code added for Setting Minimum Price....
			$arr=array("user_id"=>$user_id, "sess_id"=>session_id(), "product_id"=>$product_id, "quantity"=>$qty, "price"=>$product_price, "total_price"=>$total_price, "date_added"=>date("Y-m-d H:i:s"), "store_id"=>$store_id, "notes"=>$notes, "contact_me"=>$contact_me,'currency'=>$currency);
			
			//$arr["other_gift_category"]= ($other_gift_category == 'Y' ) ? 'Y' : 'N'; // added eldho for sepcifing that this product is added from the "other_gift" category
			
			if($pgift_id)
			{
				$arr["pgift_id"]=$pgift_id;
				$arr["product_title"]=$product_title;
			}
			...
			$flg = $this->insertCustomFieldValues($cart_id,$product_id); // eldho added on march 21st 2016
			...
			{	$source_path = $req["image_path"];
				...
				if ($req["image_del"]=="Y")
				{
					@unlink($source_path."$prd_image.".$im_ext);
				}
    		...
    		if($req['access']) {	
	    		...
	    		foreach ($req['access'] as $key=>$val) {
					...
					foreach ($newVal as $val2) {				
						if ($val2) {
							...
							### Dated  31/ March 2008.........................
							if($this->config['minimum_price']){
								$minaccessprice += $price*$qty*$area;
							}
							
							$accessory_dispaly_order='';
							
							if($accessory['type']=='art')
								$accessory_dispaly_order=1;
							elseif($accessory['type']=='poem')	
									$accessory_dispaly_order=2;
							elseif($accessory['type']=='mat')	
									$accessory_dispaly_order=3;	
							...
							$this->db->insert("cart_accessory", array("cart_id"=>$cart_id, "category_id"=>$key, "accessory_id"=>$val2, "price"=>$price, 
															  ...
															  "accessory_dispaly_order"=>$accessory_dispaly_order));
						}
				...
				### Dated  31/ March 2008.........................
				if($this->config['minimum_price']){
					$mintot=$minaccessprice+$minprodprice;
					if($mintot <$this->config['minimum_price']){
						$minpro=$this->config['minimum_price']-$minaccessprice;
						$divamt=$qty*$area;
						$minpro=$minpro/$divamt;
						$minarra=array("price"=>$minpro);
						//print_r($minarra);exit;
						$this->db->update("cart", $minarra, "id='$cart_id'");
					}
     ...
     function updateCart (&$req) {
		...
		if($req['cart']) {
			foreach ($req['cart'] as $cart_id=>$quantity) {
				...
				list($product_id,$price,$total_price,$pgift_id)	=	$this->db->get_row("select product_id,price,total_price,pgift_id from cart where id='$cart_id'",ARRAY_N);
				...
				if($objProduct->IsGiftCertificate($product_id)==false)
					{
					if($pgift_id)
					$product_price	=	$objPrice->GetPriceOfPredefinedProduct($pgift_id, 0,$quantity);
					else
				...
				$this->db->update("cart", array("quantity"=>$quantity,"price"=>$price, "total_price"=>$total_price,'cart_notes'=>$_REQUEST['notes1'],'currency'=>$currency), "id='$cart_id' AND sess_id='$session_id'");#to avoid the updation of the cart table with additional notes
    		}
			/*echo "<br>";
		exit;*/
    	}
    ...
    function deleteCart ($id="") {
	   ...
	   /*	
		...
		{
		$sql .= " AND cart.store_id='$store_id'";
		}*/
		//print_r($sql);exit;
    ...
    function getCart ($val='',$ses_id="") {	
    	...
    	/*$sql = "SELECT p.id as product_id,c.*, p.display_name as name,p.cart_name as cart_name, SUM(a.price) as accessory_price,p.brand_id as brand_id,b.brand_name as brand_name
				...
				{
				$sql = "SELECT p.id as product_id,c.*, p.display_name as name,p.cart_name as cart_name, SUM(a.price) as accessory_price,p.brand_id as brand_id,b.brand_name as brand_name,p.custom_product,p.domestic_sprice,p.domestic_sprice_addtl,p.inter_sprice,p.inter_sprice_addtl
					  FROM products p LEFT JOIN brands b on (p.brand_id=b.brand_id), cart c LEFT JOIN cart_accessory a ON(a.cart_id = c.id)
					 WHERE c.product_id = p.id
					   AND c.sess_id = '$session_id' ";
				...
				{
				$sql = "SELECT p.id as product_id,c.*, p.display_name as name,p.cart_name as cart_name, SUM(a.price) as accessory_price,p.brand_id as brand_id,b.brand_name as brand_name,pp.product_title,p.custom_product,p.domestic_sprice,p.domestic_sprice_addtl,p.inter_sprice,p.inter_sprice_addtl
					  FROM products p LEFT JOIN brands b on (p.brand_id=b.brand_id), cart c LEFT JOIN cart_accessory a ON(a.cart_id = c.id) LEFT JOIN product_predefined pp ON  c.pgift_id=pp.id
					 WHERE c.product_id = p.id
...
//print_r( $rs);exit;
    	...
    	if($rs) {
			...
			if($val=='N'){
	    		 $sql = "SELECT a.*, p.display_name as name,p.name as aname, p.cart_name as cart_name, m.category_name,p.type,p.image_extension
			...
			*/ 
    			...
    			for ($j=0; $j < count($rs[$i]->accessory); $j++) {
					...
					if($rs[$i]->accessory[$j]->type == 'art' || $rs[$i]->accessory[$j]->type == 'poem')
					$rs[$i]->accessory[$j]->accessory_name = $objCategory->GetAccessoryName($rs[$i]->accessory[$j]->accessory_id);
					else
					$rs[$i]->accessory[$j]->accessory_name = $objCategory->GetAccessoryName2($rs[$i]->accessory[$j]->accessory_id);
	    		...
	    		*/
				...
				$stot=$rs[$i]->quantity * $area * ( $rs[$i]->price + $rs[$i]->accessory_price );
    ...
    function placeOrder ($checkout,$store_id) { 
		...
		//$this->insert_test_paypal(serialize($checkout));
		//$this->insert_test_paypal($session_id+":"+$store_id);
		
    	$order_id = $this->db->insert("orders", $checkout);
		//print_r($order_id);exit;
		$_SESSION['order_id']	=	$order_id;   
    	$this->db->query("INSERT INTO order_products(id, order_id, store_id, product_id, 
    												 quantity, price, total_price, notes, contact_me,pgift_id) 
							   SELECT
									  id, '$order_id', store_id, product_id, quantity,
									  price, total_price, notes, contact_me,pgift_id
							     FROM
							    	  cart 
							    WHERE
							   		  sess_id = '$session_id' AND store_id='$store_id'");
			
		$chk=$this->checkTableExist("cart_job")	;
		$chko=$this->checkTableExist("order_products_job")	;
		
		if($chk==1 && $chko==1){
		$this->db->query("INSERT INTO order_products_job(id, order_id, jobname,height,width,materialcheck,file1_ext,file2_ext) 
							   SELECT
									  cart_id, '$order_id', jobname,height,width,materialcheck,file1_ext,file2_ext 
							     FROM
							    	  cart_job
							    WHERE
							   		  sess_id = '$session_id'");
		}
		$sqw="INSERT INTO order_accessory 
    						   SELECT op.id, 
    						   		  ca.category_id, 
    						   		  ca.accessory_id, 
    						   		  ca.price, 
		...
		/*$Row 		= 	$this->db->get_row("SELECT COUNT(*) AS OrderCount FROM orders WHERE MONTH(date_ordered) = MONTH(NOW())+1", ARRAY_A);
		 $OrderCount	=	$Row['OrderCount'];
				
	   $Todat			=	str_pad(date('d'), 2, '0', STR_PAD_LEFT);
		...
		$ThisMonth		=	str_pad(date('m'), 2, '0', STR_PAD_LEFT);
		 $OrderNumb		=	str_pad($OrderCount, 4, '0', STR_PAD_LEFT);
			if ($this->config['show_date_ordernumber']=='Y'){
				//$OrderNumber	=	$Today.''.$ThisMonth.''.$Todat.''.$OrderNumb;
				$OrderNumber = mktime(date("H"), date("i"), date("s"), date("m"),date("d"),date("Y"));
			}else{
		...
		*/						
		...
		if($_SESSION['coupon'])
		{
			...
			$arr['order_number'] = $checkout['order_number'];
			$arr['coupon_id'] = $_SESSION['coupon']['coupon_id'];
			$arr['curr_date'] = date("Y-m-d H:i:s");
			$arr['user_id'] = $checkout['user_id'];
			$arr['store_id'] = $checkout['store_id'];
			$arr['coupon_type'] = $_SESSION['coupon']['coupon_type'];
			$arr['substract_from'] = $_SESSION['coupon']['substract_from'];
			$arr['amount_discount'] = $_SESSION['coupon']['amount'];
			$arr['coupon_key'] =  $_SESSION['coupon']['key'];
			$this->saveCouponReport($arr);
		}		
    	
	 	//========================
    	$this->sendMail($order_id,$sname,$store_id);
	...
	{
		$Row 		= 	$this->db->get_row("SELECT COUNT(*) AS OrderCount FROM orders WHERE MONTH(date_ordered) = MONTH(NOW()) AND YEAR(date_ordered) = YEAR(NOW()) ", ARRAY_A);
		 $OrderCount	=	$Row['OrderCount'];
	   ...
	   $Todat			=	str_pad(date('d'), 2, '0', STR_PAD_LEFT);
		...
		$ThisMonth		=	str_pad(date('m'), 2, '0', STR_PAD_LEFT);
		 $OrderNumb		=	str_pad($OrderCount, 4, '0', STR_PAD_LEFT);			
			$Time 			=	date('his');
			if ($this->config['show_date_ordernumber']=='Y'){
				$OrderNumber	=	$Today.''.$ThisMonth.''.$Todat.''.$Time.''.$OrderNumb;
			}else{
				$OrderNumber	=	$Today.''.$ThisMonth.''.$Time.''.$OrderNumb;
			}
		return $OrderNumber;
	}
    
    function sendMail($order_id,$sname,$store_id='') {
		$user	 	=	new User();	
		global $global;
		//global $store_id;
    	...
    	$coupon_rs = $extras->historyByOrderid1($order_id, 'C');
		...
		$shipping_price_new     = "";
		$Shipp_Country_name = strtoupper( trim($ordDetails->shipping_country) );
		if(	$shipping_price>0) {
			if ( ( $global["usa_shippping_free_minvalue"] > 0 && $global["usa_shippping_free_percentage"] > 0 && $Shipp_Country_name == "UNITED STATES" ) ) {
				$shippping_free_minvalue 		= $global["usa_shippping_free_minvalue"];
				$shippping_free_percentage   	= $global["usa_shippping_free_percentage"];
			}elseif ( ( $global["canada_shippping_free_minvalue"] > 0 && $global["canada_shippping_free_percentage"] > 0 && $Shipp_Country_name == "CANADA" ) ) {
				$shippping_free_minvalue 		= $global["canada_shippping_free_minvalue"];
				$shippping_free_percentage   	= $global["canada_shippping_free_percentage"];
			}elseif ( ( $global["intl_shippping_free_minvalue"] > 0 && $global["intl_shippping_free_percentage"] > 0 ) ) {
				$shippping_free_minvalue 		= $global["intl_shippping_free_minvalue"];
				$shippping_free_percentage   	= $global["intl_shippping_free_percentage"];
			}
			
			if(	( $ordDetails->cart_price >= $shippping_free_minvalue ) && $shippping_free_minvalue>0 ) {
				$shipping_discountamt	=  round (  $shipping_price * ( $shippping_free_percentage/100 ) ,2 );
		...
		$total_amount=$sub_total + $coupon_amount + $certificate_amount+$invoice_amt;
		if ($global["special_discount_field"]=="Y"){ 
			...
			$total_amount=$sub_total + $coupon_amount + $certificate_amount+$invoice_amt;
			$email_tpl->assign("DISCOUNT",'Y');	
			$email_tpl->assign("DIS_PERC",$dsicount_perc);	
			$email_tpl->assign("DIS_AMT",$discount_amount);	
		}
		...
		$email_tpl->assign("TOTAL_AMOUNT",$total_amount);
    	...
    	if ($store_id != "")
	    	{
	    		$res = $store->storeGet($store_id);
	  			if($res['email'] !=''){
	  				$email_header['from'] 	= 	$res['email'];
				}	
				else{
					$res2 = $store->GetStoreOwnerEmailByStoreId($store_id);
	  				$email_header['from']	= 	$res2[0]['email'];
				}
	    	}
    	else
    		{
	...
	/*	$sql="select email from member_master where id='9'";
    	...
    	$email = new Email();
    	$email->send('order_confirmation', $email_header, $dynamic_vars,$store_id);
    }
/*
...
function GetTaxTitle($country_id,$state_name,$storename='')
	...
	$dynamic_vars['STORE_NAME']  = $storeDetails['heading'];
	 ...
	 $order_id=$_SESSION['order_id'];
		 ...
		 $sql ="SELECT  id  FROM orders WHERE order_number ='$txn_id'";
		 $rs =$this->db->get_row($sql,ARRAY_A);
		 $order_id=$rs['id'];
		 }
		...
		if($ordDetails->tax>0)
		{
		$dynamic_vars['TAX']	= '$'.$ordDetails->tax;
		}else
		{
		$dynamic_vars['TAX']	= 'Nil';
		}
		$dynamic_vars['AMOUNT_PAID']	= '$'.$ordDetails->paid_price;
		$dynamic_vars['PAYER_EMAIL']	= $ordDetails->billing_email;
		
		$shipping_address= $ordDetails->shipping_first_name.' '.$ordDetails->shipping_last_name.'<br>';
		$shipping_address.= $ordDetails->shipping_address1.'<br>';
		$shipping_address.= $ordDetails->shipping_address2.'<br>';
		$shipping_address.= $ordDetails->shipping_city.'<br>';
		$shipping_address.= $ordDetails->shipping_state.'<br>';
		$shipping_address.= $ordDetails->shipping_postalcode.'<br>';
		$shipping_address.= $ordDetails->shipping_country.'<br>';
		
		$shipping_address.= 'Phone: '.$ordDetails->shipping_telephone.'<br>';
		$shipping_address.= 'Mobile: '.$ordDetails->shipping_mobile.'<br>';
		
		$dynamic_vars['SHIPPING_ADDR']	= $shipping_address;
		
	 $sql ="SELECT body FROM email_config WHERE bit_type=2 and name='order_confirmation_msg'";
	 $rs =$this->db->get_row($sql,ARRAY_A);
	...
	function encodeSession()
	{
	 ...
	 function editFromCart(&$req,$cart_id) {
    	...
    	extract($req);  
			...
			$icd=$this->db->update("cart",$arr,"id='$cart_id'");	
    ...
    * Total amount in pay invoice details
	...
	/**
    * addfile extension To CartJob
  	* Author   : Jipson Thomas
  	* Created  : 17/April/2008
  	* Modified : 17/April/2008 By Jipson Thomas.
  	*/
	 function addfileextToCartJob($id,$ftype1='',$ftype2='') {
	 	$sql="UPDATE `cart_job` SET `file1_ext` = '$ftype1',`file2_ext` = '$ftype2' WHERE `cart_id` =$id ";	
		$this->db->query($sql);
		return true;
     }	
     
	/**
    * function to update cart in personalizedgift
  	* Author   : Jeffy
  	* Created  : 08/May/2008
  	* Modified : 08/May/2008 By Jeffy.
  	*/
	 
	function personalizedUpdateCart(&$req)
	{
	...
	/**
    * function used to update the cart values(for personlized gift)
  	* Author   : adarsh	
  	* Created  : 09/May/2008
  	* Modified : 08/May/2008 By adarsh.
  	*/
	 
	 function updateCartData(&$req)
	  {
		$objPrice		=	new Price();
    	$objProduct 	= 	new Product();
    	$objAccessory	=	new Accessory();
    	extract($req);  
			...
			### End of Code added for Setting Minimum Price....
			$arr=array("user_id"=>$user_id, "sess_id"=>session_id(), "product_id"=>$product_id, "quantity"=>$qty, "price"=>$product_price, "total_price"=>$total_price, "date_added"=>date("Y-m-d H:i:s"), "store_id"=>$store_id,  "contact_me"=>$contact_me);
			if($notes != ''){
				$arr["notes"]	= $notes;
			}
			...
			$this->db->update("cart",$arr,"id='$cart_id'");
			...
			$sql = "DELETE FROM cart_accessory WHERE cart_id ='$cart_id'";
    		$this->db->query($sql);								
	...
	function getAccessoryCategoryByName($name)
	{	
		$name	=	trim($name);
		$qry	=	"select T2.category_id from product_accessories T1 LEFT JOIN category_accessory T2 ON T1.id=T2.accessory_id where T1.name='$name'";
		$res	=	$this->db->get_results($qry,ARRAY_A);
		return $res;
	}
	
	function getOrderBySessId($order_sess_id)
	{
		$sql="select id from orders where order_sess_id='$order_sess_id'";
		$RS = $this->db->get_results($sql,ARRAY_A);
		return $RS;
	}

	public function saveCouponReport($req)
	{
		$this->db->insert("store_coupon_usage_history",$req);
		return $id;
		
	}
	

	public function checkOtherGiftProduct($product_id)
	{
	    	$sql 				 =	  "SELECT category_product_id   FROM category_product  WHERE product_id = '$product_id' AND category_id = 306";
	    	$rs  				 =    $this->db->get_row($sql);
	    	$category_product_id =    $rs->category_product_id;
			
			return $category_product_id > 0 ? 'Y' : 'N';
	}	
	...
	public function insert_test_paypal($str){
		$arr	=	array("vals" => $str);
    	$id		=	$this->db->insert("test_paypal", $arr );
		return $id;
	}	
