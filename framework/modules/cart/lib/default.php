...
switch ($_REQUEST['act']) {
	...
	case "delete":
		...
		$flg	=	$cart->deleteProductCustomFieldValues($_REQUEST['id']);
	...
	case "view":
		...
		if($_SERVER['REQUEST_METHOD']=="POST") {
			...
			$cart->updateCart(array_map('mysql_real_escape_string',$_REQUEST));
...
#END
		...
		$framework->tpl->assign("PROCEED_TO_CHECKOUT", createImagebutton_Div("Proceed to Checkout",'javascript:proceed();',""));
		...
		$framework->tpl->assign("CHECKOUTBUTTON_SMALL", createImagebutton_Div("Check Out","javascript:proceed();","checkout()"));
		
		$framework->tpl->assign("COUPON_KEY_SUBMIT", createImagebutton_Div("Submit","#","return keySubmit();"));
		...
		if ( $global['payment_tpl'] == 'cartlogin'){
			if(count($CARTARRAY[records])>0){
				foreach($CARTARRAY[records] as $key=>$val){
					 ...
					 $store_name = $storeName[heading];
					 $CARTARRAY[records][$key]->store_name =  $store_name;
					// $CARTARRAY[records][$key]->other_gift_category =   $cart->checkOtherGiftProduct( $val->product_id );
					 
				}
			}
		}
//		print_r($CARTARRAY);
		$framework->tpl->assign("IF_PARENT_CAT_NULL", $cartArray['records']->category_id);
		//print_r($CARTARRAY);
		...
		$framework->tpl->assign("HOME_PAGE_URL", $url);	

		
		$framework->tpl->assign("TAX_DET", $typeObj->getSaletaxDetails($store_id));
		$framework->tpl->assign("TAX_MSG", $typeObj->getSaletaxMsg());
		
		
		$total_price = $CARTARRAY['total'];
		$sess_id=session_id();
		if($sess_id)
		{
		...
		if($_POST['key'])
		{
			...
			$coupon_key = $_POST['key'];
			$res = $objcoupon->validateCoupons($coupon_key,$_POST['amount'],$store_id);
			
			//print_r($res);
										
			if($res['key_exist'] == "fail"){
				setMessage("The Coupon Key you entered does not exist or is inactive.");
				$message = "The Coupon Key you entered does not exist or is inactive.";
			}
			else if($res['date_status'] == "sdate_fail")
			{
				if ($res['date_message']) {
					setMessage("Your Coupon is inactive.");
					$message = "Your Coupon is inactive.";
				}
			}
			else if($res['date_status'] == "edate_fail")
			{
				if ($res['date_message']) {
					setMessage("Your Coupon expired on ".date("D, dS M Y", strtotime($res['date_message'])));
					$message = "Your Coupon expired on ".date("D, dS M Y", strtotime($res['date_message']));
				}
			}
			else if($res['usage_status'] == "fail")
			{
				setMessage("Your coupon usage has exceeded the allowed number of uses.");
				$message = "Your coupon usage has exceeded the allowed number of uses.";
			}
			else if($res['amount_status'] == "fail")
			{
				setMessage("You need cart total of $".$res['balance_message']." to used use this coupon");
				$message = "You need cart total of $".$res['balance_message']." to used use this coupon";
			}
			else
			{
				$substract_from	=	$res['substract_from'];//print_r($chk);
				$amount	= $res['amount'];
				$_SESSION['coupon'] = array("key"=>$_POST['key'], "amount"=>$amount, "substract_from"=>$substract_from, "coupon_type"=>$res['coupon_type'],"coupon_id"=>$res['coupon_id']);
			
			}
				
			}
			if($_SESSION['coupon']['substract_from']!="S")
			{ 
				
				if ($_SESSION['coupon']['coupon_type']=='A') {
				...
				{ 
						...
						elseif($_SESSION['coupon']['substract_from']=="SP")
						{  
						...
						elseif($_SESSION['coupon']['substract_from']=="P")
						{  
				...
				// substract from product or total
					...
					// end substract from product or total
							 ...
							 $total_price			=	round($total_price + $coupon_amount ,2);	
		...
		if($coupon_amount)
		{
			$_SESSION['coupon']['discount'] = $coupon_amount;
		
		}

		//$sql	=	"SELECT * FROM category_product where product_id =".$id;
		
		$framework->tpl->assign("ERR_MESSGAE", $message);		
		$framework->tpl->assign("TOTAL_AMT", $total_price);	
	...
	global $store_id;
		...
		if ($_SERVER['REQUEST_METHOD']=="POST") {
			if ($objUser->authenticate($_POST["txtuname"],$_POST["txtpswd"],$store_id)) {
				if(isset($_REQUEST['billinfo']) && $_REQUEST['billinfo']=='Y')
				{
					$_SESSION['order_notes']		=	$_REQUEST['notesL'];
				}
				redirect(makeLink(array("mod"=>"cart", "pg"=>"default"), "act=shipping"));
			} else {
				$err = $objUser->getErr();
...
#END
		...
		$framework->tpl->assign("main_tpl", SITE_PATH."/modules/cart/tpl/shipping.tpl");
			...
			$framework->tpl->display($global['curr_tpl']."/checkout.tpl");
			 ...
			 redirect(makeLink(array("mod"=>"cart", "pg"=>"default"), "act=view&err=1"));
			}
		}
	...
	// end of added for registering a user from shipping page on 12-11-07
			...
			if($_REQUEST['flat_rate']){
			$_SESSION['shipping_price']=$_REQUEST['flat_rate'];
			...
			redirect(makeLink(array("mod"=>"cart", "pg"=>"default"), "act=payment"));
			}
		}
		...
		if ($_SESSION['SHIPPING_ADDRESS'] == "") {
			if ($_SESSION['memberid']) {
				...
				*/	
				...
				/*
					Modified:  22/May/2008  By adarsh
					To display the details of user in shipping page(first name,last name,mobile and email)
				*/
				
				$master[0]['fname'] 	=	$userDetails['first_name'];
				$master[0]['lname'] 	=	$userDetails['last_name'];
				$master[0]['email'] 	=	$userDetails['email'];
	...
	//for populating the addrees billing address page
		if($_REQUEST['storename'])
		{
			$store_det 	= $objUser->getStoreDetails($_REQUEST['storename']);
			$store_user	= $store_det['user_id']; 
			if($store_user)
			{
				
				$store_user_details = $objUser->getUserdetails($store_user);
				
			}
		}
		$framework->tpl->assign("STORE_USER_DETAILS", $store_user_details);
	#for 3 image buttons	
...
#END
		...
		$cctext 	=	$objCms->GetCMSpageByTitle('Shipping Page');
		//print_r($cctext[0]['content']);exit;
		$framework->tpl->assign("SHIPPING_MSG", $cctext[0]['content']);
...
//print_r($_SESSION);
		...
		*/
		...
		$shipping_price_new     = "";
		$Shipp_Country_name = strtoupper( trim( $cart->getCountryNameById($_SESSION['SHIPPING_ADDRESS']['country']) ) );
		if(	$shipping_price>0) {
			...
			if(	($cartArray['total']>= $shippping_free_minvalue) && $shippping_free_minvalue>0 ) {
...
$sess_id=session_id();
	...
	//echo unserialize($test);
		...
		if(	$shipping_price>0) {
			...
			if(	($cart_price>= $shippping_free_minvalue) && $shippping_free_minvalue>0 ) {
				$shipping_discountamt	=  round (  $shipping_price * ( $shippping_free_percentage/100 ) ,2 );
				$shipping_price_new   	=  number_format($shipping_price - $shipping_discountamt,2);
			}
		}
		...
		if($_SESSION['coupon']['substract_from']=="T")
			...
			elseif($_SESSION['coupon']['substract_from']=="S")
			{  
				if($shipping_price > $_SESSION['coupon']['amount'])
				{ $coupon_amount = - $_SESSION['coupon']['amount'];}
				else
				{ $coupon_amount = - $shipping_price; }
			}
			else
			{
				if($cart_price > $_SESSION['coupon']['amount'])
		...
		// substract from product or total
			...
			elseif($_SESSION['coupon']['substract_from']=="S")
			{  
				$coupon_amount =	- $shipping_price * $_SESSION['coupon']['amount'] / 100;	
			}
			else
			{
				$coupon_amount = - $cart_price * $_SESSION['coupon']['amount'] / 100;
	  ...
	  ### Start showing Pay Invoice Nov-27-2007 by shinu ###
		...
		foreach($activepaymenttypes as $activepaymenttype) {
			...
			if($activepaymenttype['id'] == 2 && $activepaymenttype['active'] == 'Y'){
				$payment_mod[$ArrIndx++]	=	array( 'head3'=>'Pay through paypal account or credit card','url'=>SITE_PATH."/modules/cart/tpl/paypalstandardform.tpl","id"=>'4');
				}
		...
		if($framework->config['payment_receiver'] == 'store') {
			...
			//$rec_billing            =	$PaymentObj->getPayflowRec_billFromStoreName($_REQUEST['storename']);
		} else {
		...
		$framework->tpl->assign("STATE", $State2Code);
		...
		if($_REQUEST['storename']=='')		
			{
				$framework->tpl->assign("DISPLAY_STORE", 'Main Store');
			}
			else
			{
				$framework->tpl->assign("DISPLAY_STORE", $_REQUEST['storename']);
			}
		if($Country2Code == 'US')
			$State2Code	=	$objUser->getUSStateCodeFromStateName($_SESSION['BILLING_ADDRESS']['state']);
		else
	...
	/*	
		...
		$framework->tpl->assign("CART_TOTAL", $cartArray['total']);
		$framework->tpl->assign("SHIPPING_PRICE", $shipping_price > 0 ? number_format($shipping_price, 2, '.', '') : 'Free');
		$framework->tpl->assign("PAYPAL_SHIPPING_PRICE", $shipping_price > 0 ? number_format($shipping_price, 2, '.', '') : 0);
		$framework->tpl->assign("SUB_TOTAL", $sub_total);
		$framework->tpl->assign("COUPON_AMOUNT", $coupon_amount);
		$framework->tpl->assign("CERTIFICATE_AMOUNT", $certificate_amount);
		$framework->tpl->assign("TOTAL_AMOUNT", $sub_total + $coupon_amount + $certificate_amount);
		...
		if($paid_price) {
			...
			// end robin
				...
				if($Result['Approved'] == 'No') {
					...
					$msgRes = $Result['Message'];
					redirect(makeLink(array("mod"=>"cart", "pg"=>"default"), "act=payment&storename=$storename&msg=$msgRes"));
				} else {
	...
	case "coupon_check":
		...
		$memID = $_SESSION['memberid'] ? $_SESSION['memberid'] : 0;
        if ( $rs['user_id'] == $memID || $rs['user_id'] == 0 )
		echo $rs ? $rs['coupon_amounttype'] : "Coupon Key is invalid. Please try again.";
		else
		echo "Coupon Key is invalid. Please try again.";
	...
	case "thanks":
		...
		if( $_REQUEST["order_id"] != "" ) {
			$framework->tpl->assign("THANKS", $cart->listMessage($_REQUEST["order_id"] ) );
		}else {
	...
	case "paypal_process":
		 ...
		 if ($_GET['status'] == 'void') {
			...
			$new_status = "Completed";
			$brief_description = 'TEST-1-We have received an installment payment of $'.$_POST['payment_gross'].'.';
		 ...
		 $ipn_status_array['form_ipn']			= 'order';
		 $ipn_status_array['order_number']		= $item_number;
		 
		 $ipn_status_array['payer_first_name']				= $_POST['first_name'];
		 $ipn_status_array['payer_last_name']				= $_POST['last_name'];
		 $ipn_status_array['payer_address_zip']				= $_POST['address_zip'];
		 $ipn_status_array['payer_address_country_code']	= $_POST['address_country_code'];
		 $ipn_status_array['payer_address_street']			= $_POST['address_street'];
		 $ipn_status_array['order_sess_id']					= $_REQUEST['CurrentSessId'];
		 
		  $str='<table width="62%" border="0" cellspacing="0" cellpadding="0">';
		 
		 foreach($_POST as $key=>$val)
		 {
		 	$str.= '<tr>';
			$str.= '<td width="39%" align="right">'.$key.'</td>';
			$str.= '<td width="8%" align="left">:&nbsp;&nbsp;</td>';
			$str.= '<td width="57%" align="left">'.$val.'</td>';
			$str.= '</tr>';
		 }
		 $str.='</table>';
	
		 $ipn_status_array['ipn_variables']	= $str;
		 ...
		 //Add log entry ends
			...
			$order_det = $cart->getOrderBySessId($CurrentSessId);
			...
			if (count($order_det) == 0 ) {
				$checkout			=	array('user_id'				=>		$_SESSION['memberid'], 
									  ...
									  'order_sess_id'		=>		$_REQUEST['CurrentSessId']
									  );
		...
		case "paypal_process2":
			...
			$req   =  'cmd=_notify-validate&mc_gross=10.00&protection_eligibility=Ineligible&payer_id=3SK66WZDPEPPC&tax=0.00&payment_date=06%3A09%3A36+Apr+20%2C+2016+PDT';
			$req  .=  '&payment_status=Completed&charset=windows-1252&first_name=Shinu&mc_fee=0.59&notify_version=3.8&custom=&payer_status=unverified';
			$req  .=  '&business=shinu7_1280766787_biz%40gmail.com&quantity=1&verify_sign=ADvj-tIuMJxr3YEo-QPTwZC-2h.3AMKYYISnqGAG30bMlyD-5NSrcBO9';
			$req  .=  '&payer_email=shinu777%40gmail.com&txn_id=2RG10576XP578134A&payment_type=instant&last_name=T&receiver_email=shinu7_1280766787_biz%40gmail.com';
			$req  .=  '&payment_fee=0.59&receiver_id=T9XZYS2KL9Y2A&txn_type=web_accept&item_name=Personalized+Gifts+by+newagesmb+Order+No.+1604200908580020&mc_currency=USD';
			$req  .=  '&item_number=1604200908580020&residence_country=US&test_ipn=1&handling_amount=0.00&transaction_subject=&payment_gross=10.00';
			$req  .=  '&shipping=0.00&ipn_track_id=2bbd3a1911d58';
			...
			if( !($res = curl_exec($ch)) ) {
				...
				//$str	=	"ERROR - ".serialize(curl_getinfo($ch)); 
				//$str	=	"ERROR - ".$req;
				$cart->insert_test_paypal("ERROR - ".curl_error($ch));
				$cart->insert_test_paypal("ERROR - ".$req);
				...
				$cart->insert_test_paypal("OKKKK - ");
			}
			curl_close($ch);

		break;
		
		case "paypal_process1":
			//echo "Success";
			$cart->insert_test_paypal("STARTS - ".$_REQUEST['CurrentSessId']);
			//exit;
			global $store_id;
			...
			$CurrentSessId	=	$_REQUEST['CurrentSessId'];
			$cart->decodeSession($CurrentSessId);
			$test_mode_paypal =	$PaymentObj->getPayflowtest_modeFromStoreName1($_SESSION['pp_store_id']);
			...
			{
				$ch = curl_init('https://www.sandbox.paypal.com/cgi-bin/webscr');
				$paypalmode	=	"SANDBOX MODE";
			}	
			else
			{
				$ch = curl_init('https://www.paypal.com/cgi-bin/webscr');
				$paypalmode	=	"LIVE MODE";
			}	
			$header .="Connection: close\r\n\r\n";	
			curl_setopt($ch, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_1_1);
			...
			if( !($res = curl_exec($ch)) ) {
				...
				//$str	=	"ERROR - ".serialize(curl_getinfo($ch)); 
				//$str	=	"ERROR - ".$req;
				$cart->insert_test_paypal("ERROR : {$paypalmode} - ".curl_error($ch) ." - CurrentSessId : ". $_REQUEST['CurrentSessId']);
				//$cart->insert_test_paypal("ERROR - ".$req);
				curl_close($ch);
				exit;
			}else{
				$cart->insert_test_paypal("SUCCESS : {$paypalmode} - CurrentSessId : ". $_REQUEST['CurrentSessId']);
			}
			curl_close($ch);
