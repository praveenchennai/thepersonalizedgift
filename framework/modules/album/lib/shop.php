	...
	case "track_upload":
		...
		if($_SERVER['REQUEST_METHOD'] == "POST")
		{
			...
			if(!$ferror && !$_FILES['videoFile']['error']) {
				if (!$framework->maxRecordLimit($_SESSION['memberid'],'album_music'))
				{
...
/*					$mov = new ffmpeg_movie($tmpname);
					...
					if ($framework->config['media_encrypt']==1)
					{
						$media_name = md5($id);
						$arr = array();
						$arr['media_encrypt_name'] = $media_name;
                    ...
                    move_uploaded_file($_FILES['videoFile']['tmp_name'], $dir.$media_name.".".strtolower($album->file_extension($_FILES['videoFile']['name'])));
                    chmod($dir.$media_name.".".strtolower($album->file_extension($_FILES['videoFile']['name'])), 0777);
                    
                    if ($framework->config['media_ffmpeg']==1)
					{
						$start_preview = $req['start_preview'];
						if ($start_preview=="") $start_preview=0;
						if ($filetype=="") $filetype="mp3";
						$mov = new ffmpeg_movie($dir.$media_name.".$filetype");
						$featured_duration = $framework->config['featured_song_duration'];
						$other_duration    = $framework->config['other_song_duration']; 
						shell_exec("ffmpeg -ss $start_preview -t $featured_duration -i {$dir}{$media_name}.{$filetype} -acodec copy -vcodec copy -y {$dir}{$media_name}_featured.$filetype");
						shell_exec("ffmpeg -ss $start_preview -t $other_duration -i {$dir}{$media_name}.{$filetype} -acodec copy -vcodec copy -y {$dir}{$media_name}_other.$filetype");
						chmod($dir.$media_name."_featured.".$filetype, 0777);
						chmod($dir.$media_name."_other.".$filetype, 0777);
					}
	...
	case "addtocart":
		...
		if($global["admin_payment"]=="1"){
			$item_rs['total_price']=$global['track_price'];
		}
	...
	case "viewcart":
		...
		//if($file_type=="album"){
			$framework->tpl->assign("TOTAL_PRICE",$tp['amount_to_pay']);
		//}else{
		//	$framework->tpl->assign("TOTAL_PRICE", $album->cartTotal($memberID));
		//}
		if($_REQUEST['paymessage']!=""){
			setMessage($_REQUEST['paymessage']);
		}
		...
		//print_r($_POST);exit;
				...
				redirect(makeLink(array("mod"=>"album", "pg"=>"shop","sslval"=>"true"),"act=sub_credpayment&user_id=$memberID&tot_amt={$_POST['amount']}&action=shopcart_pay"));
			}
		break;
	case "removefromcart":
		checkLogin();
	...
	//	print_r($_REQUEST['pid']);exit;
		if($_REQUEST["repage"]=="photo_browse"){
			redirect(makeLink(array("mod"=>"member", "pg"=>"browse"),"act=photo&pid=".$_REQUEST['pid']));
		}elseif($_REQUEST["repage"]=="video_browse"){
			redirect(makeLink(array("mod"=>"member", "pg"=>"browse"),"act=video&pid=".$_REQUEST['pid']));
		}elseif($_REQUEST["repage"]=="photo_list"){
			redirect(makeLink(array("mod"=>"album", "pg"=>"photo"),"act=list&pid=".$_REQUEST['pid']));
		}elseif($_REQUEST["repage"]=="video_list"){
			redirect(makeLink(array("mod"=>"album", "pg"=>"video"),"act=list&pid=".$_REQUEST['pid']));
		}else{
