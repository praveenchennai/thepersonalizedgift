...
switch($_REQUEST['act']) {
	...
	case "myform_map":
		...
		if ($_REQUEST['id'] && !empty($_REQUEST['id']))	{
			...
			if ($req) {
				...
				$advDescription = addslashes($advDescription);
				...
				$popStr	.= "<div align=left class=smalltext3  style=width:80%;text-align:justify><b>" . addslashes($req['adv_title']) . "</b></div>";
	...
	case 'test_mail':
		...
		$objAdvertiser->advertisement_publish_mail('121','45');
		break;
