...
class Payment extends FrameWork 
{
	...
	function getStoreCombo()
	{
		...
		if($this->config['payment_receiver'] == 'store') {
			...
			$Qry1		=	"SELECT id,name,heading FROM store WHERE active = 'Y' ORDER BY id ASC ";
			$Result1 	= 	$this->db->get_results($Qry1);
			if($Result1) {
				$ArrIndx	=	1;
	 ...
	 * @return unknown
	 */
	function saveStore2checkoutDetails($REQUEST)
	{
		extract($REQUEST);
		
		$Qry1		=	"SELECT COUNT(*) AS ConfCount FROM store_payment_details 
						WHERE store_id = '$store_id' AND config_var = '2checkout' ";
		$Row1		=	$this->db->get_row($Qry1, ARRAY_A);
		$ConfCount	=	$Row1['ConfCount']; # If ConfCount == 0 configuration exists
		
		if($ConfCount == 0) {
			$InsArray	=	array('store_id' => $store_id, 'config_var' => '2checkout', 'config_value' => $account_number);
			$this->db->insert("store_payment_details", $InsArray);
		}
		
		if($ConfCount > 0) {
			$UpdArray	=	array('config_value' => $account_number);
			$this->db->update("store_payment_details", $UpdArray, " store_id = '$store_id' AND config_var = '2checkout' ");
		}
		
		return TRUE;
	}
	
	function saveStoreWorldpaytDetails($REQUEST)
	{
		extract($REQUEST);
		
		$Qry1		=	"SELECT COUNT(*) AS ConfCount FROM store_payment_details 
						WHERE store_id = '$store_id' AND config_var = 'Worldpayid' ";
		$Row1		=	$this->db->get_row($Qry1, ARRAY_A);
		$ConfCount	=	$Row1['ConfCount']; # If ConfCount == 0 configuration exists
		
		if($ConfCount == 0) {
			$InsArray	=	array('store_id' => $store_id, 'config_var' => 'Worldpayid', 'config_value' => $merchant_id);
			$this->db->insert("store_payment_details", $InsArray);
		}
		
		if($ConfCount > 0) {
			$UpdArray	=	array('config_value' => $merchant_id);
			$this->db->update("store_payment_details", $UpdArray, " store_id = '$store_id' AND config_var = 'Worldpayid' ");
		}
	...
	function saveStoreGoogleCheckoutDetails($REQUEST)
	{
		extract($REQUEST);
		
		$Qry1		=	"SELECT COUNT(*) AS ConfCount FROM store_payment_details 
						WHERE store_id = '$store_id' AND config_var = 'GoogleCheckoutid'|| 'GoogleCheckoutkey' ";
		$Row1		=	$this->db->get_row($Qry1, ARRAY_A);
		$ConfCount	=	$Row1['ConfCount']; # If ConfCount == 0 configuration exists
		
		if($ConfCount == 0) {
			$InsArray	=	array('store_id' => $store_id, 'config_var' => 'GoogleCheckoutid', 'config_value' => $merchant_id);
			$this->db->insert("store_payment_details", $InsArray);
		}
		if($ConfCount == 0) {
			$InsArray	=	array('store_id' => $store_id, 'config_var' => 'GoogleCheckoutkey', 'config_value' => $merchant_key);
			$this->db->insert("store_payment_details", $InsArray);
		}
		
		if($ConfCount > 0) {
			$UpdArray	=	array('config_value' => $merchant_id);
			$this->db->update("store_payment_details", $UpdArray, " store_id = '$store_id' AND config_var = 'GoogleCheckoutid' ");
		}
		
		if($ConfCount > 0) {
			$UpdArray	=	array('config_value' => $merchant_key);
			$this->db->update("store_payment_details", $UpdArray, " store_id = '$store_id' AND config_var = 'GoogleCheckoutkey' ");
		}
	 ...
	 * @return message
	 */
	function validateStore2checkoutForm($REQUEST)
	{
		extract($REQUEST);
		$Msg	=	'';
	...
	function validateStoreWorldpayForm($REQUEST)
	{
		extract($REQUEST);
		$Msg	=	'';
		
		if(trim($merchant_id) == '')
			$Msg	.=	'Merchant ID required<br>';
	...
	function validateStoreGoogleCheckoutForm($REQUEST)
	{
		extract($REQUEST);
		$Msg	=	'';
	 ...
	 * @return array
	 */
	function get2checkoutStoreAccountDetails($store_id)
	{
		$AccountDetails	=	array();
		$Qry1			=	"SELECT config_value FROM store_payment_details WHERE store_id = '$store_id' AND config_var = '2checkout'";
		$Row1			=	$this->db->get_row($Qry1, ARRAY_A);
		
		$AccountDetails['account_number']	=	$Row1['config_value'];
		
		return $AccountDetails;
	}
	
	function getWorldpayStoreAccountDetails($store_id)
	{
		$AccountDetails	=	array();
		$Qry1			=	"SELECT config_value FROM store_payment_details WHERE store_id = '$store_id' AND config_var = 'Worldpayid'";
		$Row1			=	$this->db->get_row($Qry1, ARRAY_A);
		
		$AccountDetails['merchant_id']	=	$Row1['config_value'];
		
		return $AccountDetails;
	}
	
	function getGoogleCheckoutStoreAccountDetails($store_id)
	{
		$AccountDetails	=	array();
		$Qry1			=	"SELECT config_value FROM store_payment_details WHERE store_id = '$store_id' AND config_var = 'GoogleCheckoutid'";
		$Row1			=	$this->db->get_row($Qry1, ARRAY_A);
	...
	function getGoogleCheckoutStoreAccountDetails1($store_id)
	{
		$AccountDetails	=	array();
		$Qry1			=	"SELECT config_value FROM store_payment_details WHERE store_id = '$store_id' AND config_var = 'GoogleCheckoutkey'";
		$Row1			=	$this->db->get_row($Qry1, ARRAY_A);
		
		$AccountDetails['merchant_key']	=	$Row1['config_value'];
	 ...
	 */
	function get2checkoutDetailsFromStoreName($storename = '0')
	{
		...
		$Qry2				=	"SELECT config_value FROM store_payment_details WHERE store_id = '$store_id' AND config_var = '2checkout'";
		$Row2				=	$this->db->get_row($Qry2, ARRAY_A);
	...
	function getWorldpayDetailsFromStoreName($storename = '0')
	{
		...
		$Qry2				=	"SELECT config_value FROM store_payment_details WHERE store_id = '$store_id' AND config_var = 'Worldpayid'";
		$Row2				=	$this->db->get_row($Qry2, ARRAY_A);
		$businessaccount	=	$Row2['config_value'];
	...
	function getGoogleCheckoutDetailsFromStoreName($storename = '0')
	{
		...
		if($storename == '0') {
		 ...
		 $Qry2				=	"SELECT config_value FROM store_payment_details WHERE store_id = '$store_id' AND config_var = 'GoogleCheckoutid'";
		$Row2				=	$this->db->get_row($Qry2, ARRAY_A);
		$businessaccount	=	$Row2['config_value'];
	...
	function getGoogleCheckoutDetailsFromStoreName1($storename = '0')
	{
		...
		$Qry2				=	"SELECT config_value FROM store_payment_details WHERE store_id = '$store_id' AND config_var = 'GoogleCheckoutkey'";
		$Row2				=	$this->db->get_row($Qry2, ARRAY_A);
		$businessaccount	=	$Row2['config_value'];
	...
	function saveStorePayflowLinkDetails($REQUEST)
	{
		...
		if($ConfCount == 0) {
			//$InsArray	=	array('store_id' => $store_id, 'config_var' => 'payflowlink_business_email', 'config_value' => $account_mailaddress,'test_mode'=>$test_mode,'test_mode_msg'=>$test_mode_msg_new,'currency_id'=>$currency_id);
		...
		if($ConfCount > 0) {
			...
			if ($this->config['single_prod'] == 1){
				//$UpdArray['test_mode']	=	$test_mode;
				$UpdArray['rec_billing']	=	$rec_billing;
				//$UpdArray['test_mode_msg']	=	$test_mode_msg_new;
				//$UpdArray['currency_id']	=	$currency_id;
			}
	...
	function saveStoresettingDetails($REQUEST)
	{
		extract($REQUEST);
		...
		if($ConfCount == 0) {
			//$InsArray	=	array('store_id' => $store_id, 'config_var' => 'payflowlink_business_email', 'config_value' => $account_mailaddress,'test_mode'=>$test_mode,'test_mode_msg'=>$test_mode_msg_new,'currency_id'=>$currency_id);
			$InsArray	=	array('store_id' => $store_id, 'config_var' => 'payflowlink_business_email', 'test_mode'=>$test_mode,'test_mode_msg'=>$test_mode_msg,'currency_id'=>$currency_id,'mail_authenticate'=>$mail_authenticate,'mail_domain'=>$mail_domain,'mail_username'=>$mail_username,'mail_password'=>$mail_password,'mail1_batch'=>$mail1_batch,'mail1_batchcount'=>$mail1_batchcount,'mail1_timelimit'=>$mail1_timelimit,'mail_port'=>$mail_port);
			 $this->db->insert("store_payment_details", $InsArray);
		...
		if($ConfCount > 0) {
			...
			if ($this->config['single_prod'] == 1){
				...
				//$UpdArray['rec_billing']	=	$rec_billing;
				$UpdArray	=	array('currency_id'=>$currency_id,'mail_authenticate'=>$mail_authenticate,'mail_domain'=>$mail_domain,'mail_username'=>$mail_username,'mail_password'=>$mail_password,'mail1_batch'=>$mail1_batch,'mail1_batchcount'=>$mail1_batchcount,'mail1_timelimit'=>$mail1_timelimit,'mail_port'=>$mail_port);
				$UpdArray['test_mode']	=	$test_mode;
				$UpdArray['test_mode_msg']	=	$test_mode_msg;
				$UpdArray['currency_id']	=	$currency_id;
			}//print_r($UpdArray);print($store_id);exit;
			$this->db->update("store_payment_details", $UpdArray, " store_id = '$store_id' AND config_var = 'payflowlink_business_email' ");
		}
	...
	function getPayflowLinkStoreAccountDetails($store_id)
	{
		...
		if ($this->config['single_prod'] == 1){
			...
			$AccountDetails['test_mode']	=	$Row1['test_mode'];
			$AccountDetails['rec_billing']	=	$Row1['rec_billing'];
			$AccountDetails['test_mode_msg']=	$Row1['test_mode_msg'];
			$AccountDetails['currency_id']  =	$Row1['currency_id'];
			
			$AccountDetails['mail_authenticate']  =	$Row1['mail_authenticate'];
			$AccountDetails['mail_domain'] 		  =	$Row1['mail_domain'];
			$AccountDetails['mail_username'] 	  =	$Row1['mail_username'];
			$AccountDetails['mail_password'] 	  =	$Row1['mail_password'];
			$AccountDetails['mail1_batch']  	  =	$Row1['mail1_batch'];
			$AccountDetails['mail1_batchcount']   =	$Row1['mail1_batchcount'];
			$AccountDetails['mail1_timelimit'] 	  =	$Row1['mail1_timelimit'];
			$AccountDetails['mail_port'] 	 	  =	$Row1['mail_port'];
	...
	function getPayflowtest_modeFromStoreName($storename = '0')
	{
		...
		$Qry2				=	"SELECT test_mode FROM store_payment_details WHERE store_id = '$store_id' AND config_var = 'payflowlink_business_email'";
		$Row2				=	$this->db->get_row($Qry2, ARRAY_A);
		$test_mode	=	$Row2['test_mode'];
		
		return $test_mode;
	}

	function getPayflowtest_modeFromStoreName1($store_id = '0')
	{
	...
	function getPayflowRec_billFromStoreName($storename = '0')
	{
		...
		$Qry2				=	"SELECT rec_billing FROM store_payment_details WHERE store_id = '$store_id' AND config_var = 'payflowlink_business_email'";
		$Row2				=	$this->db->get_row($Qry2, ARRAY_A);
		$rec_billing	=	$Row2['rec_billing'];
	 ...
	 * @return Boolean
	 */
	/*modified on 23-04-2008 by vipin*/
	function makeInvoicePayments($POST, $InvoiceId, $SentToId, $MemberId,$start_date,$end_date,$prop_bill_id)
	{
			...
			while (!feof($fp)) {
				...
				if (strcmp ($res, "VERIFIED") == 0) {
					...
					if ($payment_status == 'Completed') {
						...
						$this->db->query($Qry1);
						$Qry2	=	"UPDATE invoices SET invoice_status = 'PAID' WHERE invoice_interval='$prop_bill_id'";
						$this->db->query($Qry2);
						$Qry3	=	"UPDATE property_bill_detail SET bill_status = 'PAID', date_paid = '$today' WHERE prop_bill_id = '$prop_bill_id'";
						$this->db->query($Qry3);
	...
	# The following method show the total invoices of a broker or manager
	/**
      * 
      * Author   : Vipin	 
	...
	function getInvoiceListall($MemberId, $invoice_type, $day_count, $start_date,$sentby_id)
	{
			$Qry			=	"SELECT 
								T1.invoice_id AS invoice_id,
								T1.sentby_id AS sentby_id,
								...
								DATE(T1.created_time) AS created_time,
						     	sum(T1.invoice_amount) AS invoice_amount,
								T1.invoice_description AS invoice_description,
								T1.invoice_status AS invoice_status,
								T1.invoice_type AS invoice_type,
								T2.username AS SenderUsername,
							...
							LEFT JOIN member_master AS T2 ON T2.id = T1.sentby_id 
							LEFT JOIN member_paymentdetails AS T3 ON T3.memberid = T1.sentby_id 
							WHERE T1.sentto_id = '$MemberId' AND T1.deleted = 0 AND invoice_type='$invoice_type' AND T1.invoice_status = 'PENDING' and T1.sentby_id = '$sentby_id' and T1.invoice_interval=0 
						 AND date(created_time)>='$start_date' 
						 GROUP BY sentto_id  ";
			//AND created_time BETWEEN '$start_date' and DATE_ADD('$start_date',INTERVAL $day_count DAY)
		$InvoiceList	=	$this->db->get_results($Qry,ARRAY_A);
	
		return $InvoiceList;
	}
	
	function getStorePaymentMsg($store_id)
	{
		$msg='';
		$sql="select * from store_payment_details where store_id='$store_id'";
		$pay_method =	$this->db->get_results($sql, ARRAY_A);
		
		$sql="select * from shippingmethod_to_store where store_id='$store_id'";
		$shipping_method =	$this->db->get_results($sql, ARRAY_A);
		
	    $url = "<a  href=".makeLink(array("mod"=>"store", "pg"=>"home_paymentType"), "act=type&sId=Payment%20Gateway&fId=29").">Payment Gateway</a>";
		
		$url2 = "<a  href=".makeLink(array("mod"=>"store", "pg"=>"home_shipping"), "act=shippingselmethodslist&sId=Shipping&fId=33").">Shipping information</a>";

		
		/* if(count($pay_method) <= 0 && count($shipping_method)<= 0)
		$msg='Your Store is currently Active. However your users will not be able to check out until you set up '.$url.' and '.$url2.' for your store.';
		else if(count($shipping_method)<=0)
		$msg='Your Store is currently Active. However your users will not be able to check out until you set up '.$url2.' for your store.';
		else*/
		
		 if(count($pay_method)<=0)
		$msg='Your Store is currently Active. However your users will not be able to check out until you set up '.$url.' for your store.';
