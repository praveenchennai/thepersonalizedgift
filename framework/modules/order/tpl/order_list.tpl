	...
	<table border=0 width=100% cellpadding="5" cellspacing="0"> 
	  ...
	  <tr>
		...
		<td width="1%" nowrap class="naGridTitle" align="left"><input type="checkbox" name="select_all" onClick="javascript:CheckCheckAll(document.form2,'archieve_id[]')"></td> 
          ...
          <td width="5%" nowrap class="naGridTitle">{makeLink mod=$smarty.request.mod pg=$smarty.request.pg orderBy="order_status_name" display="Order Status"}act=list&date_from={$DATE_FROM}&date_to={$DATE_TO}&order_status={$CURRENT_STATUS}{/makeLink}</td>
		  
		  <td width="20%" nowrap class="naGridTitle">Payment Status</td>
          ...
          <td width="12%" align="right" nowrap class="naGridTitle">{makeLink mod=$smarty.request.mod pg=$smarty.request.pg orderBy="paid_price" display="Total Price"}act=list&date_from={$DATE_FROM}&date_to={$DATE_TO}&order_status={$CURRENT_STATUS}{/makeLink}</td>
			...
			{foreach from=$ORDER_LIST item=row}
				<tr class="{cycle values="naGrid1,naGrid2"}">
				  ...
				  <td valign="middle" height="24" align="left">{$row->ps}&nbsp;{if $row->payment_status eq 'Pending'}<img src="{$GLOBAL.tpl_url}/images/icon_small_info.gif" width="17" height="16" {popup text="`$GLOBAL.payment_status_pending_msg`" width="600" fgcolor="#eeffaa"} />{/if}</td>
				  <td valign="middle" height="24" align="center">{$row->paid_price|number_format:"2"}</td>
				</tr> 
			{/foreach}
        <tr> 
