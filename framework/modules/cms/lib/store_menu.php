...
switch($_REQUEST['act']) {
    case "list":
		...
		if($section_details['show_sub_store']=='N')
		{
		 $update_allstores='Y';
		}else
		{
        ...
        list($rs, $numpad) = $cms->storePageList($_REQUEST['section_id'],$_REQUEST['id'], $_REQUEST['pageNo'], 10, "mod=$mod&pg=$pg&act=".$_REQUEST['act']."&id=".$_REQUEST['id']."&section_id=".$_REQUEST['section_id'], OBJECT, $_REQUEST['orderBy']);
        ...
        $framework->tpl->assign("main_tpl", FRAMEWORK_PATH."/modules/cms/tpl/store_menu.tpl");
        break;
    case "delete":
