    ...
    */
    ...
    function pageList ($menu_id, $pageNo, $limit = 10, $params='', $output=OBJECT) {	
		...
		if($store_id){
			//$sql	=	"SELECT a.*,DATE_FORMAT(a.post_date, '".$this->config['date_format']." ".$this->config['time_format']."') AS post_date_f FROM cms_page a,
						...
						b.cms_id is NOT NULL AND a.post_admin='' AND b.store_id='$store_id' AND menu_id = '$menu_id' AND b.cms_type='P' ORDER BY a.position ASC, a.post_date DESC ";
    ...
    function pageAddEdit (&$req) {
        ...
        if(!trim($menu_id)) {
            ...
            $array = array("menu_id"=>$menu_id,"title"=>$title, "short_content"=>$short_content, "content"=>$content,"post_date"=>date("Y-m-d H:i:s"), "position"=>$position, "active"=>$active);
    ...
    function linkList($area, $store="",$fullpath="") {
		...
		if($store_id){
			...
			//$sql	=	"SELECT a.*,b.cms_type FROM cms_link a,store_cms b WHERE a.id=b.cms_id AND (b.cms_type='L' OR b.cms_type = 'C') AND a.area='$area' AND b.store_id='0' ORDER BY a.position";
			 ...
			 else if($area=='store_left_sub1')
			 
			  $sql	= "select * from cms_link a inner  join  category_product b on  a.category_id=b.category_id  inner  join  product_store c on b.product_id=c.product_id inner join products p on p.id= b.product_id where c.store_id='$store_id' and a.area='$area' and p.active='Y' group by  b.category_id  ORDER BY a.position";
			 
			 
		/*	 $sql	=	"SELECT a.*,b.cms_type FROM cms_link a,store_cms b left join master_category m on a.id WHERE a.id=b.cms_id AND (b.cms_type='L' OR b.cms_type = 'C') AND a.area='$area' AND b.store_id='0' ORDER BY a.position";*/
  ...
  function pageGetByName ($name, $output=ARRAY_A)
   {
	...
	# return store page details
	# created on July 9,09
	 function storePageList ($section_id='',$menu_id, $pageNo, $limit = 10, $params='', $output=OBJECT) {	
		global $store_id;
		if($store_id){
				/*$sql	= 	"SELECT a.*,DATE_FORMAT(a.post_date, '".$this->config['date_format']." ".$this->config['time_format']."') AS post_date_f FROM cms_page a LEFT JOIN store_cms b ON (a.id=b.cms_id)  WHERE 
				...
				$sql	= 	"SELECT  T3.name as menu_name,a.*,DATE_FORMAT(a.post_date, '".$this->config['date_format']." ".$this->config['time_format']."') AS post_date_f FROM cms_page a LEFT JOIN store_cms b ON (a.id=b.cms_id)  
							 LEFT JOIN cms_menu T3 ON T3.id=a.menu_id WHERE
							 b.cms_id is NOT NULL AND a.post_admin='' AND b.store_id='$store_id' AND T3.section_id = '$section_id' AND b.cms_type='P' ORDER BY T3.name ASC, a.post_date DESC ";

		}else{
			$sql	= 	"SELECT distinct a.*,DATE_FORMAT(a.post_date, '".$this->config['date_format']." ".$this->config['time_format']."') AS post_date_f FROM cms_page a LEFT JOIN store_cms b ON(a.id=b.cms_id)  WHERE 
						a.post_admin='admin' AND menu_id = '$menu_id' ORDER BY T3.name ASC, a.post_date DESC ";
		}
		//echo $sql;
		$rs = $this->db->get_results_pagewise($sql, $pageNo, $limit, $params, $output);
