...
$qry	=	"select * from products where image_extension!='' LIMIT 330,30";
$rs	=	mysql_query($qry);
while($row=mysql_fetch_array($rs))
{
	$id	=	$row["id"];
	
	# start converting images in the product/images/
	$dir			=	SITE_PATH."/modules/product/images/".$id.".".$row["image_extension"];
	
	if(file_exists($dir))
	{
	$SourceImage=$dir;
	$SourceExtension=$row["image_extension"];
	$DestinationPath=SITE_PATH."/modules/product/images/".$id.".gif";
	$DestinationExtension="gif";
		...
		if($DestinationExtension == 'GIF' || $DestinationExtension == 'gif') 
			...
			$qry2	=	"update products set image_extension='gif' where id='$id'";
			$res2	=	mysql_query($qry2);
		}
	# end converting images in the product/images
	
	
	
	# start converting images in the product/images/accessory/thumb description img
	$dir2			=	SITE_PATH."/modules/product/images/thumb/".$id."_des_.".$row["image_extension"];
	if(file_exists($dir2))
	{
	$SourceImage2=$dir2;
	$SourceExtension2=$row["image_extension"];
	$DestinationPath2=SITE_PATH."/modules/product/images/thumb/".$id."_des_.gif";
	...
	# start converting images in the product/images/accessory/thumb description img
	$dir3			=	SITE_PATH."/modules/product/images/thumb/".$id."_List_.".$row["image_extension"];
	if(file_exists($dir3))
	{
	$SourceImage3=$dir3;
	$SourceExtension3=$row["image_extension"];
	$DestinationPath3=SITE_PATH."/modules/product/images/thumb/".$id."_List_.gif";
	...
	$dir4			=	SITE_PATH."/modules/product/images/thumb/".$id.".".$row["image_extension"];
	if(file_exists($dir4))
	{
	$SourceImage4=$dir4;
	$SourceExtension4=$row["image_extension"];
	$DestinationPath4=SITE_PATH."/modules/product/images/thumb/".$id.".gif";
	...
	# deleting old files ---
	$deldir			=	SITE_PATH."/modules/product/images/".$id.".jpg";
	$deldir2			=	SITE_PATH."/modules/product/images/thumb/".$id."_des_.jpg";
	$deldir3			=	SITE_PATH."/modules/product/images/thumb/".$id."_List_.jpg";
	$deldir4			=	SITE_PATH."/modules/product/images/thumb/".$id.".jpg";
	
	if(file_exists($deldir))
	{ unlink($deldir); }
			
	if(file_exists($deldir2))
	{ unlink($deldir2); }
	
	if(file_exists($deldir3))
	{ unlink($deldir3); }
			
	if(file_exists($deldir4))
	{ unlink($deldir4); }
