 ...
 * 
 ***********************************************************************************/
class Userdb2 extends FrameWork
{
	var $ip_det = null;
	/*
	constructor
...
function insert_fluent()
	{
	     $arr_master = $this->getArrData();
	    $empl_sql = "select id from users where user_name='website' ";
		$empl_rs  = $this->db1->get_row($empl_sql);
		$empl_sql1 = "select id from test_semester  where name='".$arr_master['semester']."'";
		$empl_rs1  = $this->db1->get_row($empl_sql1);
		
		$arr_master['date_entered']=date('Y-m-d H:i:s');
		$arr_master['date_modified']=date('Y-m-d H:i:s');
		$arr_master['id']=$this->create_guid();
		$arr_master['username']=$arr_master['email'];
		$arr_master['primary_address_country']=$_REQUEST['primary_address_country'];
		$arr_master['created_by']=$empl_rs->id;
		$arr_master['modified_user_id']=$empl_rs->id;
		unset($arr_master['countryname']);
		unset($arr_master['submit_1']);
		
		if(!isset($_SESSION['guid']))
		{
		session_register('guid');
		$_SESSION['guid']=$arr_master['id'];
		}
		if($this->config['email_confirm'] =='N')
		 	$arr_master['active']=1;
		else
			$arr_master['active']=0;
	
		$this->db1->insert("test_parentgroupregistration", $arr_master);
		$id = $this->db->insert_id;
		$this->insertSemesterParent($arr_master['id'],$empl_rs1->id);
		return $id;
	}
    ...
    * Making a member active
	...
	function 	insertSemesterParent($id,$sid)
	
	{   $arr_master['id']=$this->create_guid();
		$arr_master['date_modified']=date('Y-m-d H:i:s');
		$arr_master['test_parentgroupregistration_idb']=$id;
		$arr_master['test_semester_ida']=$sid;
		$arr_master['deleted']=0;
		$this->db1->insert("test_semestregistration", $arr_master);
	
	
	}
	function makeParentActive($uid)
	{
		$arr=array();
		$arr["active"]="1";
		$this->db1->update("test_parentgroupregistration", $arr,"id='$uid'");
		return true;
	}
	...
	
	{
	   ...
	   $arr_master['date_modified']=date('Y-m-d H:i:s');
		...
		//$arr_master['modified_user_id']=$guid;
		...
		$arr_master['created_by']=$empl_rs->id;
		unset($arr_master['x']);
		unset($arr_master['submit_x']);
		unset($arr_master['submit1_x']);
		unset($arr_master['submit_y']);
		unset($arr_master['submit1_y']);
	    unset($arr_master['y']);
		unset($arr_master['add']);
		unset($arr_master['checkbox']);
		unset($arr_master['gender']);
		unset($arr_master['classtype']);
		unset($arr_master['language']);
		unset($arr_master['agelevel']);
		unset($arr_master['package']);
		unset($arr_master['abilitylevel']);
		unset($arr_master['available_daystimes']);
		unset($arr_master['email']);
		unset($arr_master['child']);
		unset($arr_master['btn_submit']);
		unset($arr_master['btn_addStudent']);
		$this->db1->insert("contacts", $arr_master);
		///$id = $this->db1->insert_id;
		//$this->insertContactCustmRelation($id,$arr_master['gender']);
		$this->insertContactRelation($arr_master['id'],$guid);
		return $arr_master['id'];
	}
	...
	function studentUpdate($sid)
	{
	   $arr_master = $this->getArrData();
		...
		unset($arr_master['email']);
		unset($arr_master['gender']);
		unset($arr_master['studentid']);
		if($arr_master['countryname']!="")
			$arr_master['primary_address_country'] = $arr_master['countryname'];
		unset($arr_master['countryname']);
		
		$this->db1->update("contacts", $arr_master,"id='$sid'");
		$this->updateContactRelation($sid);
	}
	function update_fluent($id='',$num=0)
	{

	    $arr_master = $this->getArrData();
		if($arr_master['cardnumber'])
		{
			$arr_master['cardnumber']=base64_encode($_REQUEST['cardnumber']);
		} 
		if($arr_master['hear_about_us']=='Others')
		{
		$arr_master['hear_about_us']=$arr_master['hear_about_us1'];
		$arr_master['others']=$arr_master['hear_about_us1'];
		unset ($arr_master['hear_about_us1']);
		}else
		{
		$arr_master['others']=$arr_master['hear_about_us'];
		}
		
		unset ($arr_master['hear_about_us1']);
		unset($arr_master['legal']);
		unset($arr_master['agree']);
		unset($arr_master['continue_x']);
		unset($arr_master['continue_y']);
		unset($arr_master['cpassword']);
		$this->db1->update("test_parentgroupregistration",$arr_master ,"id='$id'");
		return true;
	}
	function update_fluentscrequest($id)
	{
    	$arr_master['payment_frequency']=trim($_REQUEST['paymentfre']);
		unset($arr_master['legal']);
		unset($arr_master['agree']);
		unset($arr_master['continue_x']);
		unset($arr_master['continue_y']);
		unset($arr_master['cpassword']);
		$this->db1->update("test_screquest",$arr_master ,"modified_user_id='$id'");
		return true;
	}
   function studentDetailInsert($contact_id)
	
	{
		...
		//////////////////////
		$sql="SELECT T2.id AS semID from test_semester T2,test_parentgroupregistration T1 where T1.id='".$guid."' AND T1.semester=T2.name";
		$RS = $this->db1->get_row($sql);
		$arr_master['test_semester_id']=$RS->semID;
		
		/*print_r($arr_master['test_semester_id']);
		exit;*/
		/////////////////
		$arr_master['modified_user_id']=$guid;
		$reqstID = date('d').date('m').date('y').date('H').date('i').date('s').rand(0,999);
		$arr_master['name']=$reqstID;
		$arr_master['contact_id']=$contact_id;
		$arr_master['test_parentgroupregistration_id']=$guid;
		//$arr_master['payment_frequency']=$parent_rs->paymentfre;
		///////////need to modify 
	    ...
	    unset($arr_master['y']);
		unset($arr_master['add']);
		unset($arr_master['first_name']);
		unset($arr_master['last_name']);
		unset($arr_master['phone_work']);
		unset($arr_master['phone_home']);
		unset($arr_master['primary_address_street']);
		unset($arr_master['gender']);
		unset($arr_master['primary_address_country']);
		unset($arr_master['primary_address_state']);
		unset($arr_master['primary_address_city']);
		unset($arr_master['primary_address_postalcode']);
		unset($arr_master['birthdate']);
		unset($arr_master['email']);
		unset($arr_master['primary_address_postalcode']);
		unset($arr_master['birthdate']);
		unset($arr_master['checkbox']);
		...
		unset($arr_master['btn_submit']);
		unset($arr_master['btn_addStudent']);
		$arr_master['status']='Class Not Assigned';
		$this->db1->insert("test_screquest", $arr_master);
		
		$arr_screquest['id']=$this->create_guid();
		$arr_screquest['contacts_idb']=$contact_id;
		$arr_screquest['date_modified']=date('Y-m-d H:i:s');
		$arr_screquest['test_screquest_ida']=$arr_master['id'];
		$this->db1->insert("test_screquest_contacts",$arr_screquest);
		
		$arr_sc_mester['id']=$this->create_guid();
		$arr_sc_mester['date_modified']=date('Y-m-d H:i:s');
		$arr_sc_mester['test_semester_ida']=$RS->semID;
		$arr_sc_mester['deleted']=0;
		$arr_sc_mester['test_screquest_idb']=$arr_master['id'];
		$this->db1->insert("test_semestst_screquest",$arr_sc_mester);
		
		$arr_sc_parent['id']=$this->create_guid();
		$arr_sc_parent['date_modified']=date('Y-m-d H:i:s');
		$arr_sc_parent['test_parentgroupregistration_ida']=$guid;
		$arr_sc_parent['deleted']=0;
		$arr_sc_parent['test_screquest_idb']=$arr_master['id'];
		$this->db1->insert("test_parentst_screquest",$arr_sc_parent);
		
		return $arr_master['id'];
	}
	
	/**
    * This function is used to insert semester contact for student
  	* Author   : Ratheeshkk
  	* Created  : 27/Mar/2008
  	* 
  	*/
	function Insertsemestercontacts($contact_id,$parent_id)
	{
		$sql="SELECT T2.id AS semID from test_semester T2,test_parentgroupregistration T1 where T1.id='".$parent_id."' AND T1.semester=T2.name";
		//echo $sql;
		$RS = $this->db1->get_row($sql);
		/*print_r($RS );
		exit;*/
		$arr_semester['id']=$this->create_guid();
		$arr_semester['test_semester_ida']=$RS->semID;
		$arr_semester['contacts_idb']=$contact_id;
		$arr_semester['date_modified']=date('Y-m-d H:i:s');
		
		$this->db1->insert("test_semester_contacts", $arr_semester);
		return $RS;
	}
	...
	function InsertStudentDetails()
	{
		...
		$arr_master['date_entered']=date('Y-m-d H:i:s');
		$arr_master['id']=$this->create_guid();
		$arr_master['modified_user_id']=$_SESSION['memberid'];
		///////////need to modify 
		unset($arr_master['x']);
		unset($arr_master['submit_x']);
		unset($arr_master['submit1_x']);
		...
		$this->db1->insert("contacts", $arr_master);
		$id = $this->db1->insert_id;
		$this->insertContactRelation($arr_master['id'],$_SESSION['memberid']);
		return $id;
	}
...
function create_guid()
{
	...
	function insertContactCustmRelation($id,$guid)
	 {
	 $arr_master['id_c']=$id;
	 $arr_master['gender_c']=$guid;
	 $this->db1->insert("contacts_cstm", $arr_master);
	 }
	  function insertContactRelation($id,$guid)
	  {
		 $arr_master1 = $this->getArrData();
		 $arr_master['id']=$this->create_guid();
		 $arr_master_cstm['id_c']=$id;
		 $arr_master_cstm['gender_c']=$arr_master1['gender'];
		 $arr_master_cstm['email_c']=$arr_master1['email'];
		 $arr_master_cstm['test_parentgroupregistration_id_c']=$guid;
		 $arr_master['date_modified']=date('Y-m-d H:i:s');
		 $arr_master['test_parentgroupregistration_ida']=$guid;
		 $arr_master['contacts_idb']=$id;
		
		 $this->db1->insert("test_parention_contacts", $arr_master);
		 $this->db1->insert("contacts_cstm",$arr_master_cstm); 
		 
		 $arr_master_email['id']=$this->create_guid();
		 $arr_master_email['date_created']=date('Y-m-d H:i:s');
		 $arr_master_email['email_address']=$arr_master1['email'];
		 $arr_master_email['email_address_caps']=strtoupper($arr_master1['email']);
		 $this->db1->insert("email_addresses",$arr_master_email); 
		 
		 $arr_email_bean_rel['id']=$this->create_guid();
		 $arr_email_bean_rel['date_created']=date('Y-m-d H:i:s');
		 $arr_email_bean_rel['bean_module']="Contacts";
		 $arr_email_bean_rel['email_address_id']=$arr_master_email['id'];
		 $arr_email_bean_rel['bean_id']=$id;
		 $arr_email_bean_rel['primary_address']='1';
		 $this->db1->insert("email_addr_bean_rel",$arr_email_bean_rel); 
		 
	  }
	  function updateContactRelation($id)
	  {
		 $arr_master = $this->getArrData();
		 $arr_master_cstm['gender_c']=$arr_master['gender_c'];
		 $arr_master_cstm['email_c']=$arr_master['email_c'];
		 $this->db1->update("contacts_cstm",$arr_master_cstm,"id_c='$id'"); 
		 
		 
		 $sql="SELECT email_address_id  from email_addr_bean_rel where bean_id='".$id."' AND primary_address='1' LIMIT 0,1";
		 $RS = $this->db1->get_row($sql);
		 $email_addr_ID = $RS->email_address_id;
		 
		 $arr_master_email['email_address']=$arr_master['email'];
		 $arr_master_email['email_address_caps']=strtoupper($arr_master['email']);
		 $arr_master_email['date_modified']==date('Y-m-d H:i:s');
		
		 $this->db1->update("email_addresses",$arr_master_email,"id='$email_addr_ID'"); 
	 }
	
	/**
  	 * This function is used for changing password(for fluentstudent project)
  	 * Author   : adarsh
  	 * Created  : 1/Apr/2008
  	 */
	function changePassword($old_pass,$new_pass,$id)
	{
		$sql = "select * from test_parentgroupregistration where id='$id' and password='$old_pass'";
		$rs  = $this->db1->get_row($sql);
		if(count($rs)>0)
		{
			$arr=array();
			$arr["password"]=$new_pass;
			$this->db1->update("test_parentgroupregistration",$arr,"id='$id'");
			return true;
		}
		else
		{
...
function isValid_Fluent($mode)
	{
    extract($_REQUEST);
   $arrData = $this->getArrData();
   $sql="SELECT * from `test_parentgroupregistration` WHERE `username`='$username' and deleted=0";
   
	$Rs= $this->db1->get_results("SELECT * from `test_parentgroupregistration` WHERE `username`='$username'");

	
	
	return 	count($Rs);		/*	if(count($Rs)>0)
					{
						$this->setErr("User Name Already Exists!");
						return false;
...
if ($mode=="insert")
         ...
         ///////////////////////////
			if ($mode=="insert")
			{
				...
				if ($arrData["username"])
				{
					...
					{
						$Rs= $this->db->get_results("SELECT * from `member_master` WHERE `username`='$uname'");
			...
			$email=$arrData["email"];
			$Rs= $this->db->get_results("SELECT id from `member_master` WHERE `email`='$email'");
    ...
    * Function used for user authenticattion
	...
	function authenticate($uname,$password,$store_id=-1)
	{
		...
		$sql="select a.*,b.type from `member_master` a inner join `member_types` b on
		 a.mem_type=b.id where a.`username`='$uname' and a.`password`='$password'";
    ...
    * inserting to Profile Tables
	...
	function addEditProfile($uid,$table_name)
	{
		...
		{
			$sql = "Select id from $table_name where user_id=$uid";
			$rs  = $this->db->get_results($sql);
			if (count($rs)==0)
			{
				$arrData = $this->getArrData();
    ...
    * Fetching Details from Member Master and Custom fields using Username
  	...
  	* Modified : 30/Oct/2007 By Retheesh
  	*/
	function getUsernameDetails($uname)
	{
...
function getScreennameDetails($uname)
	...
	* Modified : 01/Jan/2008 By Ratheesh kk modified the query of search with respect to  fields
  	*/
	function userList ($pageNo, $limit = 10, $params='', $output=OBJECT, $orderBy,$stxt,$search_fields='',$search_values='',$criteria="=",$search_criteria='')
	{
		list($qry,$table_id)=$this->generateQry('member_master','d');
		if($search_fields)
		{
		...
		if($stxt=="")
		{
			$sql="SELECT m.*,ma.*,$qry,c.country_name,m.id as id,s.name AS storename FROM `member_master` m LEFT JOIN `member_address` ma on
			m.id=ma.user_id and ma.addr_type='master' left join country_master c 
			ON(ma.country = c.country_id) left join `custom_fields_list` d on m.id=d.table_key and d.table_id=$table_id 
			LEFT JOIN store s on s.id=m.from_store";
		...
		else
		{
			...
			m.id=ma.user_id and ma.addr_type='master' left join country_master c 
			ON(ma.country = c.country_id) left join `custom_fields_list` d on m.id=d.table_key and d.table_id=$table_id
			LEFT JOIN store s on s.id=m.from_store where ($search_by_fields)";
...
//print_r($sql);exit;
    ...
    * Fetching user details by username or email
  	...
  	* Modified : 02/Nov/2007 By Retheesh
  	*/	
	function getUserPass($uname,$email)
	{
		if($uname) {
    ...
    * Inserting Profile Comments
  	...
  	*/
	function insertComment($req, $Id)
	{
		extract($req);
		$cid=$_SESSION['memberid'];
		$date=date("Y-m-d H:i:s");
		$array = array("type"=>'profile',"file_id"=>$Id,"user_id"=>$cid,"comment"=>$comment,"postdate"=>$date);
	...
	*/	
	function checkEmail_parent($email,$user_id='')
	{	
		...
		$sql = "select * from test_parentgroupregistration where email='$email' AND deleted=0 AND active=1 ".$condition;
		//print_r($sql);exit;
		$rs = $this->db1->get_results($sql);
		if (count($rs)>0)
		{
			return true;
		}
	...
	*/	
	function loadSubscriptions($reg_pack='')
	{
		if ($reg_pack!="")
		{
			$sql ="SELECT c.*,a.*,c.id as sub_id FROM `reg_package` a inner join package_subscription b
...
//print_r($rs);
	...
	/**
    * This function is used to check user authentcation(for fluentstudent project)
  	* Author   : adarsh
  	* Created  : 26/Mar/2008
  	* Modified : Ratheesh KK for showing inactive messge  while an inactive user trying to login 
  	*/
	function chkUserAuthentication($uname,$password)
	{
		
		
    	$sql="select * from test_parentgroupregistration  where username='$uname' and password='$password' and deleted=0 ";
		$checkRS = $this->db1->get_results($sql);
		return $checkRS;
	
	}
	
	/**
    * This function is used to get the student list oa parent(for fluentstudent project)
  	* Author   : adarsh
  	* Created  : 26/Mar/2008
  	* 
  	*/
	function getStudentList($id)
	{
		...
		$rs = $this->db1->get_results($sql,ARRAY_A);
		return $rs;
	}
	/**
    * This function is used to get the user details of parent(for fluentstudent project)
  	* Author   : adarsh
  	* Created  : 26/Mar/2008
  	* 
  	*/
	...
	{
		$sql="SELECT * from test_parentgroupregistration where id='".$id."'";
		$RS = $this->db1->get_row($sql);
	// print_r($sql);
	//print_r($RS);
		return $RS;
	...
	/**
    * Updating values to member parentgroupregistration
  	* Author   : Ratheesh
  	* Created  : 28/Mar/2008
  	*/
	function update_parent($user_id)
	{
	    $arr_master = $this->getArrData();
		unset($arr_master['continue_x']);
		unset($arr_master['continue_y']);
		...
		$arr_master['primary_address_country']=$_REQUEST['primary_address_country'];
		unset($arr_master['countryname']);
		$this->db1->update("test_parentgroupregistration",$arr_master ,"id='$user_id'");
		return true;
	}
	
	function getStudentDet($id)
	{
		$sql="SELECT a.*,b.* from contacts a left join contacts_cstm b on b.id_c=a.id where a.id='".$id."'";
		$rs = $this->db1->get_row($sql);
		return $rs;
	}
	...
	function getStudentListAll($pageNo, $limit, $params='', $output=OBJECT, $orderBy,$id)
	{
		$sql="SELECT b.* from test_parention_contacts a right join contacts b on a.contacts_idb=b.id where a.test_parentgroupregistration_ida='".$id."'";
		$rs = $this->db1->get_results_pagewise($sql, $pageNo, $limit, $params, $output, $orderBy);
		return $rs;
	}
	
	function getStudentAllDet($id)
	{
		$sql="SELECT a.*,b.* from contacts a left join contacts_cstm b on b.id_c=a.id where a.id='".$id."'";
		$rs = $this->db1->get_row($sql,ARRAY_A);
		return $rs;
	}
	function getUserDet($id)
	{
		$sql="SELECT * from test_parentgroupregistration where id='".$id."'";
		$RS = $this->db1->get_row($sql,ARRAY_A);
		return $RS;
	...
	{
		$sql="select count(*) as num_student from contacts a inner join  test_parention_contacts b on b.contacts_idb=a.id where b.test_parentgroupregistration_ida='$id'";
		$rs = $this->db1->get_row($sql,ARRAY_A);
		return $rs['num_student'];
	}
	function getStudentList1($id,$limit)
	{
		$sql="SELECT b.* from test_parention_contacts a right join contacts b on a.contacts_idb=b.id where a.test_parentgroupregistration_ida='".$id."'";
		if($limit!='')
		{
	...
	function getStudentList2($pageNo, $limit, $params='', $output=OBJECT, $orderBy,$id)
	{
		$sql="SELECT b.*,c.gender_c from test_parention_contacts a right join contacts b on a.contacts_idb=b.id left join contacts_cstm c on c.id_c=b.id where a.test_parentgroupregistration_ida='".$id."'";
		$rs = $this->db1->get_results_pagewise($sql, $pageNo, $limit, $params, $output, $orderBy);
		return $rs;
	...
	/**
    * This function is used to get the screquest lists
  	* Author   : Salim
  	* Created  : 02/apr/2008
  	* 
  	*/
	function getDataFromScrequestTable($id)
	{
		$sql		=	"SELECT * FROM test_screquest WHERE contact_id ='". $id."'";
		$rs 	= $this->db1->get_results($sql,ARRAY_A);
		return $rs;	
	}
	function getIDFromScrequestTable($id)
	{
		$sql		=	"SELECT name FROM test_screquest WHERE contact_id ='". $id."'";
		$rs 	= $this->db1->get_results($sql,ARRAY_A);
		return $rs;	
	}
	
	/**
    * This function is used to get the enrollment details from test_screquest table
  	* Author   : Salim
  	* Created  : 02/apr/2008
  	* 
  	*/
	function getEnrollmentDetails($name)
	{
		$sql		=	"SELECT * FROM test_screquest WHERE name ='". $name."'";
		$rs 	= $this->db1->get_results($sql);
		return $rs;	
	}
	
	/**
    * This function is used to get the all classes from test_class table
  	* Author   : Salim
  	* Created  : 02/apr/2008
  	* 
  	*/
	...
	{
		$ids	=	$this->getEnrollmentDetails($id);
		
		$sql= "SELECT sc. * , cl.name,cl.id AS class_id,cl.days,cl.start_time,cl.end_time FROM test_screqut_test_class sc, test_class cl 
			WHERE sc.test_screquest_ida = '".$ids[0]->id."' AND cl.id = sc.test_class_idb AND sc.deleted = 0 ORDER BY cl.id";
		
		$rs 	= $this->db1->get_results($sql,ARRAY_A);
		return $rs;	
	}
	
	function getClassIDs($scid)
	{
		$ids	=	$this->getEnrollmentDetails($scid);
		
		$sql= "SELECT cl.id AS class_id FROM test_screqut_test_class sc, test_class cl 
			WHERE sc.test_screquest_ida = '".$ids[0]->id."' AND cl.id = sc.test_class_idb AND sc.deleted = 0 ORDER BY cl.id";
	...
	/**
    * This function is used to approve the selected class
  	* Author   : Salim
  	* Created  : 03/apr/2008
  	* 
  	*/
	function approveClasses($class_id,$screq_name)
	{
		$sc_req_arr	=	array();
		$sc_req_arr["status"]="Student Approved";
		$rs = $this->getClassNameById($class_id);
		$sc_req_arr["test_class_name"]=$rs[0][name];
		$sc_req_arr["test_class_id"]=$class_id;
		$this->db1->update("test_screquest", $sc_req_arr,"name='$screq_name'");
	}
	
	/**
    * This function is used to get the class name by given id
  	* Author   : Salim
  	* Created  : 03/apr/2008
  	* 
  	*/
	...
	{
		$sql="SELECT name FROM `test_class` WHERE `id` = '$id'";
		$RS = $this->db1->get_results($sql,ARRAY_A);
		return $RS;
	}
	
	/**
    * This function is used to get all the class details from test_class
  	* Author   : Salim
  	* Created  : 03/apr/2008
  	*/	
	function getClassDetails($cl_id)
	{
		$sql = "SELECT * FROM test_class WHERE id ='$cl_id'";
		$rs = $this->db1->get_results($sql,ARRAY_A);
		return $rs;
	}
	
	/**
    * This function is used to get all the payment details of logged in user
  	* Author   : Salim
  	* Created  : 04/apr/2008
  	*/	
	function getPaymentDetOfLoggedUser($uid)
	{
		$sql = "SELECT tp.*,c.first_name,sc.name FROM test_payments tp,contacts c,test_screquest sc 
				WHERE tp.test_parentgroupregistration_id ='$uid' AND c.id = tp.contact_id AND tp.test_screquest_id = sc.id";
		$rs = $this->db1->get_results($sql,ARRAY_A);
		return $rs;
	}
	
	/**
    * This function is used to get all the payment details of a student
  	* Author   : Salim
  	* Created  : 04/apr/2008
  	*/	
	function getPaymentDetOfStudent($uid)
	{
		$sql = "SELECT tp.*,c.first_name,sc.name FROM test_payments tp,contacts c,test_screquest sc 
				WHERE tp.contact_id ='$uid' AND c.id = tp.contact_id AND tp.test_screquest_id = sc.id";
		$rs = $this->db1->get_results($sql,ARRAY_A);
		return $rs;
	}
	...
	/**
    * This function is used to get all the student listed for the user
  	* Author   : Jeffy
  	* Created  : 03/apr/2008
  	* 
  	*/
	...
	{
		$sql="SELECT * FROM test_parention_contacts as a, contacts as b WHERE a.test_parentgroupregistration_ida = '$id' and a.contacts_idb = b.id";
		$RS = $this->db1->get_results($sql,ARRAY_A);
		return $RS;
	}
	
	/**
    * This function is used to get student info for the given id
  	* Author   : Jeffy
  	* Created  : 03/apr/2008
  	* 
  	*/
	function getChildDet($id)
	{
		$sql="SELECT * FROM contacts as a, contacts_cstm as b WHERE a.id = '$id' and b.id_c = '$id'";
		$RS = $this->db1->get_results($sql,ARRAY_A);
		return $RS;
	}
	
	/**
    * Purpose	: To Retrieve default semesters
  	* Author	: Jeffy
  	* Created	: 11/apr/2008
  	* 
  	*/
	...
	{
		$sql="SELECT name,id FROM test_semester WHERE default_semester =1";		
		$RS = $this->db1->get_results($sql,ARRAY_A);
		return $RS;
	}
