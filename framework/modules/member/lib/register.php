	...
	case "add_edit":
		...
		if(isset($_SERVER['REQUEST_METHOD'])){
			...
			if($_SERVER['REQUEST_METHOD']=="POST"){
				...
				$refferal_id=isset($_POST["refferal_id"])?$_POST["refferal_id"]:null;
				...
				if (!isset($getId)){
					...
					if($proceed==1)	{
						$strArr = array();
						$strArr["name"] = isset($_POST["store_name"])?$_POST["store_name"]:null;
						//$strArr["redirect_url"] = $_POST["redirect_url"];
						$strArr["heading"] =isset($_POST["heading"])?$_POST["heading"]:null;
	...
	$_SESSION['thnx_setup'] = $_REQUEST['thnx_setup'];
		...
		if($btn_save == 'Submit') {
			...
			if($PaymentMethod === 'Authorize.Net' && $authDetails['auth_net_arb']=='Y')
			{
				...
				//print("hai");print($SubAmount);exit;
