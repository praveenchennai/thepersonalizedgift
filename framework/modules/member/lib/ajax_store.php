...
switch ($_REQUEST["act"])
{
	...
	case "valid_store":
		...
		if(preg_match("/personaltouch/",strtolower(trim($store_name))))
		{
			echo "<strong class='form_error_message'>Invalid Store URL Name</strong>";
			exit;
		}
		if (preg_match("/http|www/", substr($store_name,0,4)) || preg_match("/couk|coin|com|\.com/", substr($store_name,-4)) )
		{
				echo "<strong class='form_error_message'>Invalid Store URL: Do not add a URL prefix or suffix such as http:, www., or .com.</strong>";
				exit;
		}
		else
		{
	...
	case 'check_promocode':
		...
		$validate_only_coupon = (int)$_REQUEST['validate_only_coupon']; // to check the click is validate coupon button
		...
		{
		$Output		=	"$CodeExists|$DiscountAmount|$RegDiscountAmount|$validate_only_coupon";
		}
		print $Output;
...
//			echo "<strong class='redClass'>It looks that you have already registered with us. If you have forgotten your user name and password. Please click here we will send you the username and password to the email address above.</strong>";
		...
		case 'subs_pack_vals':
			...
			if($sub_pack==1){
				...
				$str='<input type="hidden" name="p1" value="'.$duration.'">';
				$str.='<input type="hidden" name="t1" value="'.$type.'">';
	            $str.='<input type="hidden" name="a3" value="'.$amount.'">';
			    $str.='<input type="hidden" name="p3" value="'.$duration.'">';
	            $str.='<input type="hidden" name="t3" value="'.$type.'">';
			...
			break	;
		case 'subs_pack_vals_fixval':
		
			$sub_pack = $_REQUEST['sub_pack'];
			$sub_det = $objUser->getSubscrDetails($sub_pack);
			$user_id = $_SESSION['storeSess'][0]->id;
			$sub_fix_val = $objUser->getSubscrFixval($user_id);
			
			
			$str='';
			
			if($sub_pack==1){
				$tot = $sub_fix_val + $sub_det['fees'];
				$duration = $sub_det['duration'];
				$type = $sub_det['type'];
				
				$str='<input type="hidden" name="p1" value="'.$duration.'">';
				$str.='<input type="hidden" name="t1" value="'.$type.'">';
	            $str.='<input type="hidden" name="a3" value="'.$tot.'">';
			    $str.='<input type="hidden" name="p3" value="'.$duration.'">';
	            $str.=' <input type="hidden" name="t3" value="'.$type.'">';
				$str.= '<input type="hidden" name="amount" value="'.$tot.'">';
