...
switch($_REQUEST['act']) {
				...
				if($val->id==$cart_id)
				{
					...
					for($i=0;$i< count($artArray->accessory);$i++)
					{
						...
						if($accArray->type=='poem')
						{
							...
							{
								$opStr.='<div style="height:40px;"><span style="font-size:12px;FONT-WEIGHT:bold; ">Opening Line '.($m+1).'</span><br><span style="font-size:12px;"><span id="salim">'.gfdgfd.'</span></span></div>';
							}
							for($j=0;$j< $cl_count;$j++)
							{
								$clStr.='<div style="height:40px;"><span style="font-size:12px;FONT-WEIGHT:bold; ">Closeing Line '.($j+1).'</span><br><span style="font-size:12px;"><span id="salim">'.gfdgfd.'</span></span></div>';
							}
	...
	case "list_accessories_ajax":
		...
		list($res, $numpad1, $cnt, $limitList)	=	$objAccessory->accessoryLists($category_id,$store_id,$pageNo,$show,$param,OBJECT, $orderBy,'','','',$artname);
		...
		<span class="toplinkpagi">'.$numpad1.'</span>
			...
			for($i=0;$i<count($res);$i++)
			{
				...
				$str.='<span class="product"><div style="height:10px; "></div>
	...
	case "list_poem":
		...
		<span>'.$numpad.'</span>
			...
			for($i=0;$i<count($res);$i++)
			{
				...
				$str.='<div  style="margin-left:5%;width:650px;float:left;height:565px;padding:5px;overflow:auto;">
						<div align="center" style="height:25px; width:600px;" ><pre class="blacktext_fordefault" ><font size="+1">'.$res[$i]->name	.'</font></pre></div>
						<div style="width:600px;float:left;padding:5px;text-align:center;overflow:auto;">
						<pre class="blacktext_fordefault" >'.$ajax_editor->GetPoemText($res[$i]->poem);
	...
	case"load_txt":
		...
		if($req['opt'] || $req['cl'])
		{
			...
			$str.='<div>&nbsp;</div><div style=" padding-left:55px">'.createImagebutton_Div("Apply","javascript:void(0);","loadPic();").'</div>';
			$str.='</div>';
			}
		echo $str;
		break;	
	...
	break;
	 ...
	 case "load_surname":
           ...
           */
	 		...
	 		$strtoret	=	'<div id="surdiv" style="text-align:center; margin-top:-420px;"><div style="margin: 0pt auto; width: 530px; z-index:1000; position:absolute; height:280px; margin-left:100px;">&nbsp;</div><div style="font-family:Georgia; font-size:9px; width:350px; text-align:justify;margin:0 auto;color:#000000" >'.substr($surnamearry->Text,0,1000).' ...<i>continued</i></div>';
...
/* For getting parent id when parent_cat_id is NULL in URL	*/
	...
	if($_REQUEST['cart_id'])
	$framework->tpl->assign("SUBMITBUTTON2", createImagebutton_Div("Update Cart","JavaScript:void(0);","return updateCart($product_id); "));
	else
	$framework->tpl->assign("SUBMITBUTTON2", createImagebutton_Div("Add to Cart","JavaScript:void(0);","addtocart($product_id);"));
