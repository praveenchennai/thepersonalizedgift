...
class Extras extends FrameWork {
  ...
  function Extras($coupon_no="", $coupon_start="",$coupon_end="",$coupon_amount="",$coupon_amounttype="",$coupon_options="",$one_times="",$coupon_createddate="") {
	...
	function couponGet ($id) {
		...
		$SubscrQuery	=	"SELECT 
								T1.deduction_amount AS deduction_amount,
								T2.name AS SubscriptionName
							FROM coupon_subpackages AS T1 
							LEFT JOIN subscription_master AS T2 ON T2.id = T1.subscription_id 
							WHERE T1.coupon_id = '$id'";
		$Subscriptions	=	$this->db->get_results($SubscrQuery, ARRAY_A);
		$rs['Subscriptions']	=	$Subscriptions;
		
		$SubscrQuery	=	"SELECT 
								T1.deduction_amount AS deduction_amount,
								T2.package_name AS SubscriptionName
							FROM coupon_regpackages AS T1 
							LEFT JOIN reg_package AS T2 ON T2.id = T1.regpack_id 
							WHERE T1.coupon_id = '$id'";
		$Subscriptions	=	$this->db->get_results($SubscrQuery, ARRAY_A);
		$rs['Packages']	=	$Subscriptions;
	...
	function couponAddEdit (&$req) {
		...
		*/
		...
		/*else if($coupon_amounttype == 'P' && ( (float)$coupon_amount <= 0 || (float)$coupon_amount > 100 && count($SelSubscriptions) == 0) ){
			$message = "Deduction Percentage must be less than or equal to 100%";
		} else if($coupon_amounttype == 'A' && (float)$coupon_amount <= 0  && count($SelSubscriptions) == 0){
			$message = "Deduction amount must be greater than zero";
		} else if($coupon_amounttype != 'F' && $substract_from == '' ){
			$message = "Deducted Source not selected";
		} else if($coupon_amounttype != 'F' && $substract_from == 'S' && count($SelSubscriptions) == 0 ){
			$message = "Subscription packages not selected";
		} */

		if($message == '') #There is no validtion error
		{
			$array = array("coupon_name"=>$coupon_name,"coupon_no"=>$inscoupon_no, "coupon_start"=>$coupon_start, "coupon_end"=>$coupon_end, 
							...
							"coupon_options"=>$coupon_options,"one_times"=>$one_times,"coupon_createddate"=>$coupon_createddate,"substract_from"=>$substract_from, 'available' => 'Y');
					if($id) {
				unset($arry['coupon_no']);
				$array['id'] 	= 	$id;
				$this->db->update("coupons", $array, "id='$id'");
				$this->addCouponSubscriptions($_REQUEST, $req['SelSubscriptions'], $id, $substract_from, $coupon_amounttype);
				$this->addCouponPackages($_REQUEST, $req['SelPackage'], $id, $substract_from, $coupon_amounttype);
			} else { 
...
function couponList () {
	...
	function validateCoupons($trans_no,$trans_amount){		
		...
		if($rs['substract_from']=='T'){
				$array['substract_from']		=	'T';
		}
		elseif($rs['substract_from']=='O')
		{
			$array['substract_from']		=	'O';
		}
		elseif($rs['substract_from']=='M')
		{
			$array['substract_from']		=	'M';
		}
		else{
				$array['substract_from']		=	'P';
		}
		// end
	...
	function historyByOrderid1($order_id,$trans_type){
		if($trans_type=='C'){
			$query	=	"select b.coupon_type as coupon_amounttype,amount_discount as trans_useamount   from orders a
							inner join store_coupon_usage_history b on a.id = b.order_id
	 ...
	 */
	function getAllPackages($CouponId = '')
	{
		$Qry1			=	"SELECT T1.id AS id, T1.package_name AS name, T2.id AS CouponPackageId, T2.deduction_amount AS deduction_amount 
							FROM reg_package AS T1 
							LEFT JOIN coupon_regpackages AS T2 ON T2.regpack_id = T1.id AND T2.coupon_id = '$CouponId' 
							WHERE T1.active = 'Y' ";
		$SubScriptions	=	$this->db->get_results($Qry1, ARRAY_A);
		return $SubScriptions;
	}
	...
	/**
	 * The follwowing method adds and edits the Coupon subscriptuion selected packages
	 * 
	 * @author vimson@newagesmb.com
	 * 
	 */
	function addCouponSubscriptions($REQUEST, $SelSubscriptions, $CouponId, $substract_from, $coupon_amounttype)
	{
		$Qry1	=	"DELETE FROM coupon_subpackages WHERE coupon_id = '$CouponId'";
		$this->db->query($Qry1);
		
		if($substract_from != 'S' || $coupon_amounttype == 'F')
			return TRUE;
		
		foreach($SelSubscriptions as $SelSubscription) {
			$DeductionAmount	=	$REQUEST['deductionamount_'.$SelSubscription];
			$InsArr				=	array('coupon_id' => $CouponId, 'subscription_id' => $SelSubscription, 'deduction_amount' => $DeductionAmount );
			$this->db->insert('coupon_subpackages', $InsArr);
		}
		
		return TRUE;
	}
	
	function addCouponPackages($REQUEST, $SelPackages, $CouponId, $substract_from, $coupon_amounttype)
	{ 
		$Qry1	=	"DELETE FROM coupon_regpackages WHERE coupon_id = '$CouponId'";
		$this->db->query($Qry1);
		
		if($substract_from != 'S' || $coupon_amounttype == 'F')
			return TRUE;
		
		foreach($SelPackages as $SelPackage) {
			$DeductionAmount	=	$REQUEST['packagedeductionamount_'.$SelPackage];
			$InsArr				=	array('coupon_id' => $CouponId, 'regpack_id' => $SelPackage, 'deduction_amount' => $DeductionAmount );
			$this->db->insert('coupon_regpackages', $InsArr);
		}
	...
	/**
	 * The following method checks the validity of the promotion code
	 *
	 * @author vimson@newagesmb.com
	 * @param String $PromotionCode
	 * 
	 * @return Boolean	if promo code exists return success, else false
	 * 
	 */
	function validatePromotionCode($PromotionCode, $SubPack, $UserId, $objUser)
	{
		$ResultArray	=	array();
		
		if($SubPack > 0 && $SubPack != '') {
			$SubscriptionId	=	$SubPack;
		} else {
			$UserDetails	=	$objUser->getUserdetails($UserId);
			$SubscriptionId	=	$UserDetails['sub_pack'];
		}
		
		$Qry			=	"SELECT 
								T1.deduction_amount AS coupon_amount, 
								T2.coupon_amounttype AS coupon_amounttype,
								T3.name AS SubscriptionName,
								T3.fees AS SubscriptionFees
							FROM coupon_subpackages AS T1 
							INNER JOIN coupons AS T2 ON T2.id = T1.coupon_id 
							INNER JOIN subscription_master AS T3 ON T3.id = T1.subscription_id 
							WHERE T2.coupon_no = '$PromotionCode' AND T1.subscription_id = '$SubscriptionId'
							AND (CURDATE() BETWEEN coupon_start AND coupon_end) 
							AND (T2.coupon_amounttype = 'A' OR T2.coupon_amounttype = 'P') 
							AND T2.active = 'Y' AND T2.substract_from = 'S' AND T2.available = 'Y' 
							AND CASE WHEN T2.coupon_options = 'unlimit' THEN TRUE  
								WHEN T2.coupon_options = 'fixed' AND T2.one_times > 0  THEN TRUE 
								WHEN T2.coupon_options = 'one' THEN TRUE 
								ELSE FALSE END ";
		...
		if($Row) {
			...
			$coupon_amounttype		=	$Row['coupon_amounttype'];
			$SubscriptionName		=	$Row['SubscriptionName'];
			$SubscriptionFees		=	$Row['SubscriptionFees'];
			
			if($coupon_amounttype == 'P')
				$DiscountAmount		=	($SubscriptionFees * $coupon_amount) / 100;
			
			if($coupon_amounttype == 'A')
				$DiscountAmount		=	$coupon_amount;
			
			$SubscriptionExists		=	'YES';
		...
		$DiscountAmount		=	($DiscountAmount > $SubscriptionFees) ? $SubscriptionFees : $DiscountAmount;
		$DiscountAmount		=	sprintf("%0.2f",$DiscountAmount);
		
		return array($SubscriptionExists, $DiscountAmount);
	}
	
	/**
	 * The following method checks the validity of the promotion code for a registraiin package
	 *
	 * @author Shinu
	 * @param String $PromotionCode
	 * 30 Jan 2009
	 ...
	 * 
	 */
	function validatePromotionCodeforRegPack($PromotionCode, $RegPack, $UserId, $objUser)
	{ 
		$ResultArray	=	array();
		
		if($RegPack > 0 && $RegPack != '') {
			$PackageId	=	$RegPack;
		} else {
			$UserDetails	=	$objUser->getUserdetails($UserId);
			$PackageId		=	$UserDetails['reg_pack'];
		}
		
		$Qry			=	"SELECT 
								T1.deduction_amount AS coupon_amount, 
								T2.coupon_amounttype AS coupon_amounttype,
								T3.package_name AS SubscriptionName,
								T3.fee AS SubscriptionFees
								FROM coupon_regpackages AS T1 
								INNER JOIN coupons AS T2 ON T2.id = T1.coupon_id 
								INNER JOIN reg_package AS T3 ON T3.id = T1.regpack_id 
								WHERE T2.coupon_no = '$PromotionCode' AND T1.regpack_id = '$PackageId'
								AND (CURDATE() BETWEEN coupon_start AND coupon_end) 
								AND (T2.coupon_amounttype = 'A' OR T2.coupon_amounttype = 'P') 
								AND T2.active = 'Y' AND T2.substract_from = 'S' AND T2.available = 'Y' 
								AND CASE WHEN T2.coupon_options = 'unlimit' THEN TRUE  
	...
	/**
	 * The following method updates the coupon table so that if a particular use a promocode for subscription fee discount
	 *
	 * @param Array $REQUEST
	 * @return Boolean
	 * 
	 */
	function updatePromoCodeUsage($REQUEST)
	{
		$UpdArray	=	array();
		
				
		$promo_code				=	trim($REQUEST['promo_code']);
		$subscription_discount	=	(float)$REQUEST['subscription_discount'];
						
		if($subscription_discount <= 0 || $promo_code == '')
			return FALSE;
				
		$Qry		=	"SELECT * FROM coupons WHERE coupon_no = '$promo_code'";
		$Coupon		=	$this->db->get_row($Qry, ARRAY_A);
				
			
		if(empty($Coupon))
			return FALSE;
							
		if($Coupon['substract_from'] != 'S' || $Coupon['coupon_amounttype'] == 'F')
			return FALSE;
					
		$CouponId		=	$Coupon['id'];
		$one_times		=	$Coupon['one_times'];
		$coupon_options	=	$Coupon['coupon_options'];
					
		if($coupon_options == 'unlimit')
			return TRUE;
		
		if($coupon_options == 'one') {
			$UpdArray	=	array_merge($UpdArray, array('available' => 'N'));
			$this->db->update("coupons", $UpdArray, "id='$CouponId'");
			return TRUE;
		}
		
		if($coupon_options == 'fixed') {
			$RemainingValidity	=	$one_times - 1;
									
			if($RemainingValidity == 0)
				$UpdArray	=	array_merge($UpdArray, array('available' => 'N'));
			
			$UpdArray	=	array_merge($UpdArray, array('one_times' => $RemainingValidity));
			$this->db->update("coupons", $UpdArray, "id='$CouponId'");
	...
	/**
	 * The following method deactivates the promo code enetred [validated] by a particular user
	 *
	 * @param Object $objUser
	 * @param Int $UserId
	 * @return Boolean
	 */
	function deActivateUserPromoCode($objUser, $UserId)
	{
		$UpdArr	=	array('promocode_status' => 'INACTIVE' , 'id' => $UserId);
