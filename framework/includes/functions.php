...
function authorize_store() {
	...
	if($_SESSION['storeSess']){
		...
		if($endDate < $cur_date)
		{
			$_SESSION['store_renew_msg']=1;			
		
			if( $res['subscr_eot']=='Y')
			{
				$_SESSION['store_renew_msg']=0;
			...
			{
				$arr = get_time_difference($endDate,$cur_date);
				if($arr['days']>7)
				{
					$_SESSION['store_renew_msg']=1;
					if(!$_SESSION['url_redirect']){
	...
	if (SEO_PRODUCT=="true")
	{
		...
		if ($module=="product" && $page=="list")
		{
			...
			if ($collection["act"] =="listproduct")
			{
				$vars = explode("&",$content);
				$id = $collection['id'];
				$selectQuery="SELECT seo_name,custom_product from products where id=$id";
				...
				{
					$url = $rs['seo_name'].".htm";
				}else if( $rs['custom_product'] == 'Y' ){
				
					$url =	"index.php?mod=product&pg=list&act=other_gifts&id={$collection[id]}&cat_id={$collection[cat_id]}";
				}	
			}
		...
		//by eldho to make seor url for personalized gift
		if($params['mod']	== 'product' && $params['pg'] == 'list' )
		{ 
				$cont_ar1 = explode("&",$content);
				$size = sizeof($cont_ar1);
				$str	=	"";
				$str2='';
				for ($i=0;$i<$size;$i++)
				{
					$fin_ar = explode("=",$cont_ar1[$i]);
					if($fin_ar[0]=='act')
					{
						$act	=	$fin_ar[1];	
					}
				...
				if( ($act == 'pg_details_product') &&  ($id > 0) ){ 
						$PredefinedGiftSeourl	=	getPredefinedGiftSeourl($id);
						if($PredefinedGiftSeourl != '')
							$url			=	$PredefinedGiftSeourl;	
				}
			  if( ($act == 'pg_list') &&  ($cat_id > 0) ){ 
						$categorySeourl	=	getcategorySeourl($cat_id);
						if($categorySeourl != '')
							$url			=	$categorySeourl.".cat";	
			   }
		}	
...
function getcategorySeourl($id)
{
	global $framework;
	$qry		=	"SELECT seo_url FROM master_category WHERE category_id='$id'";
	$Row1		=	$framework->db->get_row($qry, ARRAY_A);
	return $Row1['seo_url'];
}
function getPredefinedGiftSeourl($id)
{
	global $framework;
	...
	$cat_name	= preg_replace('/[^\w\d_ -]/si','',$Row1['category_name']);
	$cat_name	= strtolower(str_replace(" ","-",$cat_name));

	$gift_name	= preg_replace('/[^\w\d_ -]/si','',$Row1['product_title']);
	$gift_name	= strtolower(str_replace(" ","-",$gift_name));
	
	//$url		=	$cat_name.".cat/".$gift_name.".".$id; 
	$url		=	$gift_name.".".$id;
	return $url;
}
...
function thumbnail($image_path,$thumb_path,$image_name,$MAX_WIDTH,$MAX_HEIGHT,$mode="",$new_name="",$img_adjust=1)
{
	...
	$image_name		=	str_replace(".jpg.jpg",".jpg",$image_name);
	$image_name		=	str_replace(".png.png",".png",$image_name);
	$image_name		=	str_replace(".jpeg.jpeg",".jpeg",$image_name);
