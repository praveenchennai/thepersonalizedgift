...
class DB
{
	 ...
	 * @return mixed
	 */
    function get_results_pagewise($query=null, $page, $limit = 20, $params='', $output=OBJECT, $orderBy='',$type='') {
        if ( $query ) {
            $page 		= $page ? $page : 1;
            ...
            if($page > 1) {
                $str .= '<a href="?sess='.base64_encode('pageNo='.($page-1).$params.'&limit='.$limit).'">&laquo; Prev</a> &nbsp;<font color=black>|</font> &nbsp;';
            } else {
            ...
            for($i=$start; $i<=$end; $i++) {
                if ($i == $page) {
                    ...
                    $str .= '<a href="?sess='.base64_encode('pageNo='.$i.$params.'&limit='.$limit).'">'.$i.'</a> &nbsp;';
					$str1 .= '<a href="?sess='.base64_encode('pageNo='.$i.$params.'&limit='.$limit).'">'.$i.'</a> &nbsp;';
                }
				if($i<$end){
					$str1 .= '| &nbsp;';
				}
            ...
            if($page < $numpages) {
                $str .= '<font color=black>|</font> &nbsp;<a href="?sess='.base64_encode('pageNo='.($page+1).$params.'&limit='.$limit).'">Next &raquo;</a> &nbsp;';
				$str1 .= '<font color=black>|</font> &nbsp;<a href="?sess='.base64_encode('pageNo='.($page+1).$params.'&limit='.$limit).'">Next &raquo;</a> &nbsp;';
            } else {
            ...
            if($count == 0) {
            	...
            	$arr = array(5, 10, 20, 50, 100, 200);
           		$limitList = "<select onchange=\"window.location.href='?sess=".base64_encode('pageNo=1'.$params)."&limit='+this.value;\">";
            	foreach ($arr as $val) {
			...
			if($type=='1')
				$str=$str1;
            return array($result, $str, $count, $limitList);
        }
    }
