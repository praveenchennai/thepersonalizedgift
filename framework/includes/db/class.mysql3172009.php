...
class DB
{
	 ...
	 * @param mode $output
	 * @return mixed
	 * Modified by: Retheesh Kumar on 20/Jun/2008
	 * Added new code for taking count of all records
	 */
    function get_results_pagewise($query=null, $page, $limit = 20, $params='', $output=OBJECT, $orderBy='',$type='', $pageVarName="pageNo") {
        if ( $query ) {
           ...
           // $count 		= $this->query($query);
              ...
              // New code for taking Count of Records added by Retheesh Kumar --- START -----
            
	            $sql = $query;
	            $sql = strtolower($sql);
	        if (strstr($sql,"group by") || strstr($sql,"distinct"))
	        {
	        	$count = $this->query($query);
	        }
	        else 
	        {
	            if (substr_count($sql,"from")>1)
	            {
	            	...
	            	while($valid==0)
	            	{
	            		$filter = ltrim(strstr($sql,"from"));
	            		$new_val = ltrim(substr_replace($filter,"",0,4));
	            		if ((substr($new_val,0,1)!=",") && (substr($new_val,0,4)!="from") && (substr($new_val,0,2)!="as"))
	            		{
	            			$valid = 1;
	            			$sql = $filter;
	            		}
	           ...
	           $sql_rs = $this->get_row($sql,ARRAY_A);
	           $count = $sql_rs['cnt'];
	        }    
     	   
           //New code for Count --- END -----	
    ...
    function insert($table_name, $field_array) {
	 ...
	 // modified on 26-03-08 to remove special characters
	  $fulldomainpath	= 	$_SERVER['REQUEST_URI']; 
	  $domainpath 		=	substr($fulldomainpath, 0, 6);  
		if($domainpath != "/admin")
	    {
			$qry	=	"select * from special_characters";
			$res	=	$this->get_results($qry,ARRAY_A);
			for($i=0;$i<count($res);$i++)
			{
				$character	=	$res[$i]['character'];
				$replace	=	$res[$i]['replace'];
				$field_array	=	str_replace($character,$replace, $field_array);
			}
		}
		// end modified on 26-03-08 to remove special characters
     ...
     * Aneesh Aravindan
     */
     function get_results_pagewise_ajax($query=null,$page,$limit=10, $params='',$output=OBJECT, $orderBy='',$numpadtype='', $pageVarName="pageNo", $custFun = "")
	{	
		...
		if ( $query ) {	
			...
			if($previous < 0){
				...
				if($numpadtype==1) {
					$anc .= "<a class='toplinkpagi' style='text-decoration:none' href='javascript:pagination($previous,$limit,$orderBy,1$custFun);void(0);'>Previous l </a>";
					if($page_showing>$norepeat+2)
					$anc .= "<a class='toplinkpagi' style='text-decoration:none' href='javascript:pagination(0,$limit,$orderBy,1$custFun);void(0);'>1... l </a>";
					else
					$anc .= "<a class='toplinkpagi' style='text-decoration:none' href='javascript:pagination(0,$limit,$orderBy,1$custFun);void(0);'>1 l </a>";
				}else {
					$anc .= "<a class='toplinkpagi' style='text-decoration:none' href='javascript:pagination(0,$limit,$orderBy,1$custFun);void(0);'>First l </a>";
					$anc .= "<a class='toplinkpagi' style='text-decoration:none' href='javascript:pagination($previous,$limit,$orderBy,1$custFun);void(0);'>Previous l </a>";
				}
				
				$pre_link = "<a class='toplinkpagi' style='text-decoration:none' href='javascript:pagination($previous,$limit,$orderBy,1$custFun);void(0);'>&lt;&lt;Previous </a>";
			}
			...
			for($i=$page_showing; $i>1; $i--){
				...
				if($numpadtype==1) {
					if($fpreviousPage!=1)
					$anch = "<a class='toplinkpagi' style='text-decoration:none' href='javascript:pagination($page,$limit,$orderBy,1$custFun);void(0);'>$fpreviousPage l </a>".$anch;
				}else{
					$anch = "<a class='toplinkpagi' style='text-decoration:none' href='javascript:pagination($page,$limit,$orderBy,1$custFun);void(0);'>$fpreviousPage l </a>".$anch;
				}
			...
			for($i=$page_showing; $i<$total_page; $i++){
				...
				if($numpadtype==1) {
					...
					$anc .= "<a class='toplinkpagi' style='text-decoration:none' href='javascript:pagination($page,$limit,$orderBy,1$custFun);void(0);'>$fnextPage l </a>";
				}else{
					$anc .= "<a class='toplinkpagi' style='text-decoration:none' href='javascript:pagination($page,$limit,$orderBy,1$custFun);void(0);'>$fnextPage l </a>";
				}
			...
			if($next >= $numrows){
				...
				if($numpadtype==1) {
					...
					$anc .= "<a class='toplinkpagi' style='text-decoration:none' href='javascript:pagination($last,$limit,$orderBy,1$custFun);void(0);'>$total_page l </a>";
					else 
					$anc .= "<a class='toplinkpagi' style='text-decoration:none' href='javascript:pagination($last,$limit,$orderBy,1$custFun);void(0);'>...$total_page l </a>";
					
					$anc .= "<a class='toplinkpagi' style='text-decoration:none' href='javascript:pagination($next,$limit,$orderBy,1$custFun);void(0);'>Next</a>";
				}else {
					$anc .= "<a class='toplinkpagi' style='text-decoration:none' href='javascript:pagination($next,$limit,$orderBy,1$custFun);void(0);'>Next l </a>";
					$anc .= "<a class='toplinkpagi' style='text-decoration:none' href='javascript:pagination($last,$limit,$orderBy,1$custFun);void(0);'>Last</a>";	
				}
				
				$next_link="<a class='toplinkpagi' style='text-decoration:none' href='javascript:pagination($next,$limit,$orderBy,1$custFun);void(0);'>Next&gt&gt;</a>";
			}
			...
			if ($numrows>0) {
				$arr = array(15=>15, 30=>30);  #, 45=>45, 60=>60, 90=>90, 150=>150,($numrows+1)=>All
           		$limitList = "<select id=numRecRes class=inputelement onchange='javascript:pagination(0,this.value,$orderBy,1$custFun);void(0);'>";
