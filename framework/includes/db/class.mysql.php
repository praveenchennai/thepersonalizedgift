...
class DB
{
    ...
    function query($query){
        ...
        if ( preg_match("/^(insert|delete|update|replace)\s+/i",$query) ){
            ...
            $i=0;
            while ($i < @mysql_fetch_field($this->result)){
          ...
          //  exit;
            ...
            try {
                if(!$query=='select * from member_address'){
    ...
    function get_results($query=null, $output = OBJECT)
    {
        ...
        if ($output == OBJECT){
            ...
            if($this->last_result){
                ...
                // echo 'testing'; print_r($new_array);
                // exit;
                return $new_array;
            } else {
                return null;
	 ...
	 * @return integer
	 */
    function insertIgnore($table_name, $field_array, $condition="") {
	
        $str = "INSERT IGNORE INTO `$table_name` SET ";
        if( is_array($field_array) ) {
            foreach ($field_array as $field=>$value) {
               $str .= "`$field` = '$value',";
            }
