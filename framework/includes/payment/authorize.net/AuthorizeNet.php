...
{	
	...
	function getAuthorizeNet($StoreId, $PaymentReceiver)
	{
		...
		$Qry2		=	"SELECT payconfig_varname,payconfig_varvalue FROM payment_configuration WHERE store_id = '$StoreId' AND 
						(payconfig_varname = 'auth_net_login_id' OR payconfig_varname = 'auth_net_tran_key' OR payconfig_varname = 'auth_net_arb')";
	...
	function setAuthorizeNet($REQUEST, $FILES)
	{
		...
		if($TotCount == 0) {
			...
			$InsArray3	=	array("store_id" => $store_id, "payconfig_varname" => "auth_net_arb", "payconfig_varvalue" => $auth_net_arb);
			...
			$Qry4	=	"UPDATE payment_configuration SET payconfig_varvalue = '$auth_net_arb' WHERE store_id = '$store_id' AND payconfig_varname = 'auth_net_arb'";
			$this->db->query($Qry3);
		}
	}
