...
{literal}
	<SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript" >
		function submit_form(){
			console.log('document', document.getElementById('frmReg').action)
			if(checkLength()){
		...
		*/
		...
		function loadSub(reg_pack)
		{
			//mem_type
			document.getElementById("div_sub").style.display="block";
			document.getElementById('sub_div').innerHTML="<strong class='lbClass'>{/literal}{$MOD_VARIABLES.MOD_LABELS.LBL_LOADIND}{literal}</strong>";
			var req1 = newXMLHttpRequest();
		...
		function validStore(store_name)
		{
			...
			{
				{/literal}
				document.getElementById("msg_div").innerHTML="<img src='{$GLOBAL.tpl_url}/images/blue_light.gif' border='0' height='23'/><strong style='color:#990000'>{$MOD_VARIABLES.MOD_LABELS.LBL_CHECKING}</strong>";
				var req1 = newXMLHttpRequest();
			...
			{
				document.getElementById("msg_div").innerHTML="<strong class='lbClass'>{/literal}{$MOD_VARIABLES.MOD_LABELS.LBL_MINIMUM_STORE_CHAR}{literal}</strong>";
		...
		function validEmail(email){
			if (emailCheck(email)){
				{/literal}
				document.getElementById("email_div").innerHTML="<strong style='color:#990000'>{$MOD_VARIABLES.MOD_LABELS.LBL_CHECKING}</strong>";
				var req1 = newXMLHttpRequest();
				...
				{/literal}
				document.getElementById("email_div").innerHTML="<font color='red'><strong class='redClass'>{$MOD_VARIABLES.MOD_LABELS.LBL_INVALID_EMAIL}</strong></font>";
				{literal}
			}
		}
		...
		function validUname(uname){
			...
			{
				{/literal}
				document.getElementById("uname_div").innerHTML="<strong style='color:#990000'>{$MOD_VARIABLES.MOD_LABELS.LBL_CHECKING}</strong>";
				var req1 = newXMLHttpRequest();
				...
				{/literal}
				document.getElementById("uname_div").innerHTML = "<font color=''><strong class='lbClass'>({$MOD_VARIABLES.MOD_HINTS.HT_MINIMUM_CHAR1})</strong></font>";
				{literal}
			}
		}
...
{literal}
	<SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript" >
		...
		var fields=new Array('first_name','last_name','email','username','password','confirm_pass','address1','city','country','postalcode','telephone');
		var msgs=new Array('First Name','Last Name','Email','Username','Password','Confirm Password','Address','City','Country','Zip Code','Telephone');
		...
		function checkLength(){
			console.log('testing', document.frmReg)
		...
		function checkLength(){
			...
			if (chk(document.frmReg)){	 			
				if(document.frmReg.id.value==''){
					...
					if (str1.length<4){
						{/literal}
							alert("{$MOD_VARIABLES.MOD_JS.JS_USERNAME_SHORT}");{literal}
							return false;		
					} else if (str2.length<6){
						{/literal}
							alert("{$MOD_VARIABLES.MOD_JS.JS_PASSWORD_LENGTH_SHORT}");
...
<div style="width:auto; ">
	<div class="greyboldext" >{if $smarty.request.id}{$MOD_VARIABLES.MOD_HEADS.HD_ACCOUNT_DET}{else}{$MOD_VARIABLES.MOD_HEADS.HD_REGISTRATION}{/if}</div>
	<br>
	...
	<div class="row" id="email_main" >
		...
		<span id="sub_div" class="formw" style="width:80%">
		 <input name="email" type="text" class="input" id="email" value="{$smarty.request.email}" size="30" {if ($smarty.request.id!="")} readonly {else} onBlur="validEmail(this.value)" {/if}/> &nbsp;
	...
	<div class="row">
		...
		<input name="username" type="text" class="input" id="username" value="{$smarty.request.username}" size="30" onBlur="validUname(this.value)"/>
                       <span id="uname_div"> <strong class="formw">({$MOD_VARIABLES.MOD_HINTS.HT_MINIMUM_CHAR1})</strong></span>
  	 </span>
	</div><br>
	...
	<div class="row">
		...
		<span id="sub_div" class="formw">
		<div id="div_state" class="bodytext"><input name="state" type="text" class="ajaxinput" id="state" value="{$smarty.request.state}"/></div>
  	 </span>
...
{if $smarty.request.country==""}	
 <SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript" >show_state('state',document.frmReg.country.value,'{$smarty.request.state}');</SCRIPT> 	
{/if}
</form>