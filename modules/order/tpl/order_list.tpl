...
<table align="center" width="90%" border="0" cellspacing="0" cellpadding="0" class=naBrdr> 
  <tr> 
    <td><table border="0" cellspacing="0" cellpadding="4" width="100%" align="center"> 
        <tr>
		  <td height="30" nowrap class="naH1">Orders</td> 
  ...
  <tr> 
    <td><table border=0 width=100% cellpadding="5" cellspacing="0"> 
        {if count($ORDER_LIST) > 0}
        <tr>
          <td width="13%" nowrap class="naGridTitle" height="24" align="left">{makeLink mod=order pg=order orderBy="id" display="Order #"}act=list&date_from={$DATE_FROM}&date_to={$DATE_TO}{/makeLink}</td> 
          <td width="40%" nowrap class="naGridTitle">{makeLink mod=order pg=order orderBy="billing_first_name" display="Customer"}act=list&date_from={$DATE_FROM}&date_to={$DATE_TO}{/makeLink}</td>
          <td width="15%" nowrap class="naGridTitle">{makeLink mod=order pg=order orderBy="order_status_name" display="Order Status"}act=list&date_from={$DATE_FROM}&date_to={$DATE_TO}{/makeLink}</td>
          <td width="17%" nowrap class="naGridTitle">{makeLink mod=order pg=order orderBy="date_ordered" display="Date Ordered"}act=list&date_from={$DATE_FROM}&date_to={$DATE_TO}{/makeLink}</td>
          <td width="15%" align="right" nowrap class="naGridTitle">{makeLink mod=order pg=order orderBy="paid_price" display="Total Price"}act=list&date_from={$DATE_FROM}&date_to={$DATE_TO}{/makeLink}</td>
        </tr>
        {foreach from=$ORDER_LIST item=row}
        <tr class="{cycle values="naGrid1,naGrid2"}"> 
          <td valign="middle" height="24" align="left"><a class="linkOneActive" href="{makeLink mod=order pg=order}act=details&id={$row->id}{/makeLink}">{$row->id}</a></td> 
        ...
        {/foreach}
        <tr> 
          <td colspan="4" class="msg" align="center" height="30">{$ORDER_NUMPAD}</td> 
        </tr>
        {else}
         <tr class="naGrid2"> 
