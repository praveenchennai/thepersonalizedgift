  ...
  <tr> 
    <td><table width="98%" align="center"> 
        <tr> 
          ...
          <td nowrap align="right" class="titleLink"><a href="#" onClick="showSearch();return false;">Search</a> | <a href="{makeLink mod=newsletter pg=subscription}act=form{/makeLink}">Add New</a></td> 
        </tr> 
      </table></td> 
  </tr> 
  <tr>
  ...
  <tr> 
    <td><table border=0 width=100% cellpadding="5" cellspacing="0"> 
        ...
        <tr>
          ...
          <td width="5%" nowrap class="naGridTitle" height="24" align="center"></td> 
          <td width="45%" nowrap class="naGridTitle" height="24" align="left">{makeLink mod=newsletter pg=subscription orderBy="name" display="Mailing List"}act=list&list_id={$smarty.request.list_id}&email={$smarty.request.email}&format={$smarty.request.format}&confirmed={$smarty.request.confirmed}&active={$smarty.request.active}{/makeLink}</td> 
          <td width="45%" nowrap class="naGridTitle" height="24" align="left">{makeLink mod=newsletter pg=subscription orderBy="email" display="Email"}act=list&list_id={$smarty.request.list_id}&email={$smarty.request.email}&format={$smarty.request.format}&confirmed={$smarty.request.confirmed}&active={$smarty.request.active}{/makeLink}</td> 
        </tr>
        {foreach from=$SUBSCRIPTION_LIST item=subscription}
        <tr> 
          <td valign="middle" class="{cycle values="naGrid1,naGrid2" advance=false}" height="24" align="center"><a class="linkOneActive" href="{makeLink mod=newsletter pg=subscription}act=form&id={$subscription->id}{/makeLink}"><img title="Edit" alt="Edit" src="{$GLOBAL.tpl_url}/images/grid/icon.edit.gif" border="0"></a></td> 
          <td valign="middle" class="{cycle values="naGrid1,naGrid2" advance=false}" height="24" align="center"><a class="linkOneActive" href="{makeLink mod=newsletter pg=subscription}act=delete&id={$subscription->id}{/makeLink}"onclick="javascript: return confirm('Are you sure to delete?')"><img title="Delete" alt="Delete" src="{$GLOBAL.tpl_url}/images/grid/icon.delete.gif" border="0"></a></td> 
          <td valign="middle" class="{cycle values="naGrid1,naGrid2" advance=false}" height="24" align="left">{$subscription->name}</td> 
          <td valign="middle" class="{cycle values="naGrid1,naGrid2"}" height="24" align="left"><a class="linkOneActive" href="{makeLink mod=newsletter pg=subscription}act=form&id={$subscription->id}{/makeLink}">{$subscription->email}</a></td> 
        </tr> 
        {/foreach}
        <tr> 
