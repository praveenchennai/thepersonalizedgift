...
class ImageManager 
{
	...
	function getThumbnail($relative) 
	{
		...
		_ddt( __FILE__, __LINE__, "getThumbnail(): top with '$relative' and backend '" . $this->config['backend_url'] . "'" );
		...
		Return $this->config['backend_url'] . '__function=thumbs&img='.rawurlencode($relative);
	}
