...
function EnterParagraphs(editor) 
  ...
  this.editor = editor;
	...
	this.ddt._ddt( "enter-paragraphs.js","105", "EnterParagraphs(): constructor" );
  ...
  if (HTMLArea.is_gecko)
		{
		this.ddt._ddt( "enter-paragraphs.js","114", "EnterParagraphs(): we are gecko. Setting event handler." );
...
EnterParagraphs.prototype.forEachNode = function (left_node, right_node, mode, direction, init) 
	...
	this.ddt._ddt( "enter-paragraphs.js","223", "forEachNode(): top - mode is:" + mode + "' direction '" + direction + "'" );
  ...
  while ( walk != direction == "ltr" ? right_node : left_node ) 
    ...
    if ( !walk ) 
      ...
      if ( getChild(walk,direction) ) 
		...
		this.ddt._ddt( "enter-paragraphs.js","336", "forEachNode(): calling function" );
	...
	this.ddt._ddt( "enter-paragraphs.js","383", "forEachNode(): returning false." );
...
EnterParagraphs.prototype._fenCullIds = function ( ep_ref, node, pong ) 
	...
	this.ddt._ddt( "enter-paragraphs.js","471", "_fenCullIds(): top" );
  ...
  if ( node.id ) 
		...
		this.ddt._ddt( "enter-paragraphs.js","478", "_fenCullIds(): node '" + node.nodeName + "' has an id '" + node.id + "'" );
...
EnterParagraphs.prototype.processSide = function( rng, search_direction) 
	...
	this.ddt._ddt( "enter-paragraphs.js","501", "processSide(): top search_direction == '" + search_direction + "'" );
  ...
  while ( roam = roam ? ( next(roam,search_direction) ? next(roam,search_direction) : roam.parentNode ) : start )
    ...
    if ( next(roam,search_direction) ) 
			...
			this.ddt._ddt( "enter-paragraphs.js","544", "processSide(): Checking next node '" + next(roam,search_direction).NodeName + "' for _pExclusions list." );
      ...
      if ( this._pExclusions.test(next(roam,search_direction).nodeName) ) 
				...
				this.ddt._ddt( "enter-paragraphs.js","551", "processSide(): Node '" + next(roam,search_direction).NodeName + "' is on the _pExclusions list. Stopping before it." );
        ...
        return this.processRng(rng, search_direction, roam, next(roam,search_direction), (search_direction == "left"?'AfterEnd':'BeforeBegin'), true, false);
			...
			this.ddt._ddt( "enter-paragraphs.js","559", "processSide(): No next node, examing parent node '" + roam.parentNode.nodeName + "' for containers or exclusions." );
      ...
      if (this._pContainers.test(roam.parentNode.nodeName)) 
				...
				this.ddt._ddt( "enter-paragraphs.js","566", "processSide(): Parent Node '" + roam.parentNode.nodeName + "' is on the _pContainer list. Stopping inside it." );
        ...
        return this.processRng(rng, search_direction, roam, roam.parentNode, (search_direction == "left"?'AfterBegin':'BeforeEnd'), true, false);
				...
				this.ddt._ddt( "enter-paragraphs.js","573", "processSide(): Parent Node '" + roam.parentNode.nodeName + "' is on the _pExclusion list." );
        ...
        if (this._pBreak.test(roam.parentNode.nodeName)) 
					...
					this.ddt._ddt( "enter-paragraphs.js","580", "processSide(): Parent Node '" + roam.parentNode.nodeName + "' is on the _pBreak list." );
          ...
          return this.processRng(rng, search_direction, roam, roam.parentNode,
					...
					this.ddt._ddt( "enter-paragraphs.js","588", "processSide(): Parent Node '" + roam.parentNode.nodeName + "' is not on the _pBreak list." );
	...
	this.ddt._ddt( "enter-paragraphs.js","608", "processSide(): bottom" );
...
EnterParagraphs.prototype.processRng = function(rng, search_direction, roam, neighbour, insertion, pWrap, preBr) 
	...
	this.ddt._ddt( "enter-paragraphs.js","635", "processRng(): top - insertion arg is: '" + insertion + "'" );

  var node = search_direction == "left" ? rng.startContainer : rng.endContainer;
  var offset = search_direction == "left" ? rng.startOffset : rng.endOffset;

	var pterminator = null;
  ...
  if ( search_direction == "left")
    ...
    newRng.setStart(node, offset);
		...
		this.ddt._ddt( "enter-paragraphs.js","671", "processRng(): right - new range start is '" + offset + "' end offset is '" + newRng.endOffset + "'" );
	  }
  ...
  if ( cnt.nodeType == 11 && !cnt.firstChild ) 
	...
	this.ddt._ddt( "enter-paragraphs.js","718", "processRng(): find_fill in cnt." );
  ...
  if ( fill && 
		...
		this.ddt._ddt( "enter-paragraphs.js","729", "processRng(): pify handling. Creating p tag followed by nbsp tag" );
    ...
    if ((search_direction == "left" ) && pify.previousSibling) 
			...
			this.ddt._ddt( "enter-paragraphs.js","744", "processRng(): returning created roam AfterEnd" );
			...
			this.ddt._ddt( "enter-paragraphs.js","751", "processRng(): returning created roam BeforeBegin" );
			...
			this.ddt._ddt( "enter-paragraphs.js","758", "processRng(): returning created roam for direction '" + search_direction + "'" );
  ...
  if ( fill ) 
    ...
    if ( (fill.nodeType == 1 && !this._elemSolid.test()) || fill.nodeType == 11 ) 
			...
			pterminator = editor._doc.createElement( 'p' );
			...
			this.ddt._ddt( "enter-paragraphs.js","803", "processRng(): inserting a br tag before." );
	...
	if ( fill )
	  ...
	  // And stuff a shiny new object with whatever contents we have
		...
		this.ddt._ddt( "enter-paragraphs.js","831", "processRng(): creating p tag or document fragment - pWrap is '" + pWrap + "' " );
  ...
  if (preBr) 
		{
		this.ddt._ddt( "enter-paragraphs.js","839", "processRng(): appending a br based on preBr flag" );
...
EnterParagraphs.prototype.isNormalListItem = function(rng)
	...
	if (( typeof node.nodeName != 'undefined') &&
		...
		this.ddt._ddt( "enter-paragraphs.js","882", "isNormaListItem(): node is a list item" );
		...
		this.ddt._ddt( "enter-paragraphs.js","893", "isNormaListItem(): parent is a list item" );
		...
		{
		this.ddt._ddt( "enter-paragraphs.js","902", "isNormaListItem(): not list item" );
	...
	if ( ! listNode.previousSibling )
		{
		this.ddt._ddt( "enter-paragraphs.js","914", "isNormaListItem(): we are the first li." );

		// are we on the first character of the first li? 

		if ( rng.startOffset == 0 )
			{
			this.ddt._ddt( "enter-paragraphs.js","920", "isNormaListItem(): we are on the first character." );

			return false;
			}
		}

	this.ddt._ddt( "enter-paragraphs.js","926", "isNormaListItem(): this is a normal list item case." );
...
EnterParagraphs.prototype.__onKeyPress = function(ev) 
	...
	this.ddt._ddt( "enter-paragraphs.js","940", "__onKeyPress(): top with keyCode '" + ev.keyCode + "'" );
  ...
  if (ev.keyCode == 13 && ev.ctrlKey && this.editor._iframe.contentWindow.getSelection)
		{
		this.ddt._ddt( "enter-paragraphs.js","947", "__onKeyPress(): calling handleEnter" );

    return this.handleEnter(ev);
		}

	this.ddt._ddt( "enter-paragraphs.js","952", "__onKeyPress(): bottom" );
...
EnterParagraphs.prototype.handleEnter = function(ev) 
	...
	this.ddt._ddt( "enter-paragraphs.js","967", "handleEnter(): top" );
  ...
  var rng = this.editor._createRange(sel);
	...
	if ( this.isNormalListItem(rng) )
		{
		this.ddt._ddt( "enter-paragraphs.js","983", "handleEnter(): we are at the end of a list with a blank item. Letting the browser handle it." );
		return true;
		}
  ...
  // Grab ranges for document re-stuffing, if appropriate
	...
	this.ddt._ddt( "enter-paragraphs.js","998", "handleEnter(): calling processSide on left side." );
  ...
  var pStart = this.processSide(rng, "left");
	...
	this.ddt._ddt( "enter-paragraphs.js","1004", "handleEnter(): calling processSide on right side." );
  ...
  rng.deleteContents();
	...
	this.ddt._ddt( "enter-paragraphs.js","1021", "handleEnter(): looking for cursor position" );
  ...
  if ( pStart ) 
		...
		this.ddt._ddt( "enter-paragraphs.js","1035", "handleEnter(): inserting pEnd" );
  ...
  if ( pEnd && pEnd.nodeType != 1) 
		...
		this.ddt._ddt( "enter-paragraphs.js","1043", "handleEnter(): inserting pEnd" );
  ...
  if ((holdEnd) && (this._permEmpty.test(holdEnd.nodeName) )) 
		...
		this.ddt._ddt( "enter-paragraphs.js","1053", "handleEnter(): looping to find cursor element." );
