...
// sanitize $lang to prevent adding of shell commands or additional options. 

if ( ! preg_match( "/^[a-zA-Z0-9_]+$/", $lang ) )
	{

	// we have a breach attempt.
...
// this forces the personal dictionary to be placed under the current directory.
//
// The original design allowed the dictionary path to be passed in via a POST 
// variable.

$p_dicts_path = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'personal_dicts';


if ( file_exists($p_dicts_path) && is_writable($p_dicts_path))
	{
	...
	if ( ! preg_match( "/^[a-zA-Z0-9_]+$/", $_REQUEST['p_dicts_name'] ) )
		{
	...
	$p_dict_path = $p_dicts_path . DIRECTORY_SEPARATOR . $_REQUEST['p_dicts_name'];
	...
	if ( !file_exists($p_dict_path) )
		...
		}	// end of p_dict_path didn't exist.
	...
	}	// end of if p_dicts_path exists and is writable.

// as an additional precaution check the aspell_args for illegal 
// characters

if ( preg_match( "/[|><;\$]+/", $aspell_args ) )
	{
